...
import { informationOriginLabel, informationOrigins, type InformationOrigin } from "../lib/information-status";
...
export function EvidenceRecordEditor({ record, mutate, reload, onDismiss }: { record: GovernanceRecord; mutate: Mutate; reload: () => Promise<void>; onDismiss: () => void }) {
  const [draft, setDraft] = useState({ recordType: record.recordType, status: record.status, informationOrigin: record.informationOrigin, externalReference: record.externalReference || "", title: record.title, owner: record.owner || "", occurredAt: record.occurredAt || "", participants: record.participants || "", dueDate: record.dueDate || "", summary: record.summary || "", decisionAsk: record.decisionAsk || "", actionItems: record.actionItems || "", impact: record.impact || "", adjudicationAuthority: record.adjudicationAuthority || "", adjudicatedAt: record.adjudicatedAt || "", adjudicationRationale: record.adjudicationRationale || "" });
  ...
  return <ViewportModal onDismiss={onDismiss} dismissDisabled={saving} labelledBy="edit-evidence-title" className="call-note-modal">
    ...
    <p>Update the record and its hard links. Set the information origin from the record&apos;s provenance; a lifecycle value alone does not turn a source claim into a Government decision. Removed links and files remain documented in audit history.</p>
    <div className="form-grid"><label className="modal-field">Record type<select value={draft.recordType} onChange={(event) => setDraft({ ...draft, recordType: event.target.value as GovernanceRecordType })}>{governanceRecordTypes.map((item) => <option key={item} value={item}>{displayStatus(item)}</option>)}</select></label><label className="modal-field">Information origin<select value={draft.informationOrigin} onChange={(event) => setDraft({ ...draft, informationOrigin: event.target.value as InformationOrigin })}>{informationOrigins.map((item) => <option key={item} value={item}>{informationOriginLabel(item)}</option>)}</select><small>Who supplied or recorded this information.</small></label><label className="modal-field">Record lifecycle<select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value as GovernanceRecordStatus })}>{governanceRecordStatuses.map((item) => <option key={item} value={item}>{displayStatus(item)}</option>)}</select></label><label className="modal-field">External reference<input value={draft.externalReference} onChange={(event) => setDraft({ ...draft, externalReference: event.target.value })} /></label><label className="modal-field">Owner<input value={draft.owner} onChange={(event) => setDraft({ ...draft, owner: event.target.value })} /></label><label className="modal-field">Occurred<input type="date" value={draft.occurredAt} onChange={(event) => setDraft({ ...draft, occurredAt: event.target.value })} /></label><label className="modal-field">Response due<input type="date" value={draft.dueDate} onChange={(event) => setDraft({ ...draft, dueDate: event.target.value })} /></label></div>
    {draft.recordType === "decision" ? <section className="decision-principle"><strong>Government decision details</strong><p>Required before an approved decision can be saved. These fields identify the adjudication; they do not alter the underlying source material.</p><div className="form-grid"><label className="modal-field">Decision authority<input value={draft.adjudicationAuthority} onChange={(event) => setDraft({ ...draft, adjudicationAuthority: event.target.value })} /></label><label className="modal-field">Decision date<input type="date" value={draft.adjudicatedAt} onChange={(event) => setDraft({ ...draft, adjudicatedAt: event.target.value })} /></label><label className="modal-field field-span">Decision rationale<textarea rows={3} value={draft.adjudicationRationale} onChange={(event) => setDraft({ ...draft, adjudicationRationale: event.target.value })} /></label></div></section> : null}
