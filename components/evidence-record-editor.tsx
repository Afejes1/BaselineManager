...
export function EvidenceRecordEditor({ record, mutate, reload, onDismiss }: { record: GovernanceRecord; mutate: Mutate; reload: () => Promise<void>; onDismiss: () => void }) {
  ...
  return <ViewportModal onDismiss={onDismiss} dismissDisabled={saving} labelledBy="edit-evidence-title" className="call-note-modal">
    ...
    <section className="object-link-picker"><div className="section-toolbar"><div><span className="eyebrow">HARD LINKS</span><h3>Related objects</h3></div><span>{selected.length} linked</span></div><label className="search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find product, MCP, Objective, Platform, release, Initiative, or WBS package" /></label><div className="object-link-results">{visible.map((item) => { const key = `${item.kind}|${item.id}`; return <label key={key} className={selected.includes(key) ? "selected" : ""}><input aria-label={`Link ${item.label}`} type="checkbox" checked={selected.includes(key)} onChange={() => toggle(item)} /><span><strong>{item.label}</strong><small>{displayStatus(item.kind)} · {item.detail}</small></span></label>; })}</div></section>
