...
export function GovernedImportReview({ items, decisions, targets = {}, targetOptions = [], targetLabel = "Canonical record", busy = false, applyLabel, onDecision, onTarget, onBulkDecision, onApply }: Props) {
  ...
  const unresolvedApproved = approvedItems.filter((item) => onTarget && item.disposition !== "add" && !(targets[item.id] || item.proposedTargetId));
  ...
  return <section className="domain-section governed-import-review">
    ...
    {unresolvedApproved.length ? <p className="error-copy">{unresolvedApproved.length} approved row{unresolvedApproved.length === 1 ? " requires" : "s require"} a canonical mapping.</p> : null}
