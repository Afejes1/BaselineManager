...
import type {
  ...
  SolutionKnockOn, SolutionKnockOnClassification, SolutionOption, SolutionOptionAssessment, SolutionOptionChangeRequestLink, SolutionOptionObjectiveLink, SolutionOptionStatus, SolutionOptionStep, SolutionOptionType,
  SolutionStepDependency, SolutionStepDependencyType, SolutionStepReference, SolutionStepReferenceKind,
...
type InitiativeRow = { id: string; title: string; status: string; priority: string; owner: string | null; target_date: string | null; consequence: string | null; desired_outcome: string | null; decision_ask: string | null; decision_question: string | null; as_is_statement: string | null; to_be_statement: string | null; success_measures: string | null; briefing_audience: string | null; decision_needed_by: string | null; problem_statement: string | null; drivers_constraints: string | null; rom_hours_per_point: number | null; rom_conversion_rationale: string | null; closed_at: string | null; primary_release_id: string | null; primary_release_name: string | null; updated_at: string };
...
type ObjectiveRow = { id: string; change_request_id: string | null; external_system: string; external_identifier: string; external_item_type: string; title: string; summary: string | null; source_description: string | null; government_synopsis: string | null; description_authority: "reported" | "analyst_transcribed" | "migrated_unclassified"; technical_owner: string | null; status: ObjectiveStatus; planned_start: string | null; planned_finish: string | null; actual_start: string | null; actual_finish: string | null; source_locator: string | null; source_as_of: string | null; updated_at: string };
...
type SolutionStepRow = { id: string; option_id: string; title: string; description: string | null; expected_result: string | null; parent_step_id: string | null; wbs_code: string | null; owner: string | null; planning_start: string | null; planning_finish: string | null; planning_effort_hours: number | null; planning_effort_basis: string | null; sort_order: number; updated_at: string };
type SolutionStepReferenceRow = { id: string; option_id: string; step_id: string; reference_kind: SolutionStepReferenceKind; source_id: string | null; reference: string | null; label: string; rationale: string | null; updated_at: string };
type SolutionStepDependencyRow = { id: string; option_id: string; predecessor_step_id: string; successor_step_id: string; relationship: SolutionStepDependencyType; lag_days: number; rationale: string; updated_at: string };
type SolutionKnockOnRow = { id: string; option_id: string; classification: SolutionKnockOnClassification; affected_kind: string | null; affected_id: string | null; affected_reference: string | null; timing: string | null; likelihood: EstimateConfidence; impact: EstimateConfidence; confidence: EstimateConfidence; narrative: string; mitigation: string | null; updated_at: string };
...
export async function initiativeDecisionWorkspace(db: Database, actor: Actor, evidenceScope: EvidenceVerificationScope = {}): Promise<InitiativeDecisionWorkspace> {
  ...
  const [initiativeResult, linkResult, objectiveResult, objectiveChangeRequestLinkResult, dependencyResult, attributionResult, estimateResult, feedSourceResult, requirementResult, criterionResult, signoffResult, initiativeEvidenceResult, milestoneResult, solutionOptionResult, solutionStepResult, solutionStepReferenceResult, solutionStepDependencyResult, solutionChangeLinkResult, solutionObjectiveLinkResult, solutionKnockOnResult, solutionAssessmentResult, solutionDecisionResult, solutionDecisionRevisionResult] = await Promise.all([
    ...
    db.prepare("SELECT r.* FROM solution_step_reference r JOIN solution_option o ON o.id=r.option_id JOIN initiative i ON i.id=o.initiative_id WHERE i.program_id=? ORDER BY r.option_id,r.step_id,r.updated_at").bind(PROGRAM_ID).all<SolutionStepReferenceRow>(),
    db.prepare("SELECT d.* FROM solution_step_dependency d JOIN solution_option o ON o.id=d.option_id JOIN initiative i ON i.id=o.initiative_id WHERE i.program_id=? ORDER BY d.option_id,d.updated_at").bind(PROGRAM_ID).all<SolutionStepDependencyRow>(),
    ...
    db.prepare("SELECT k.* FROM solution_option_knock_on k JOIN solution_option o ON o.id=k.option_id JOIN initiative i ON i.id=o.initiative_id WHERE i.program_id=? ORDER BY k.option_id,k.classification,k.updated_at").bind(PROGRAM_ID).all<SolutionKnockOnRow>(),
  ...
  const initiatives: InitiativeDecisionProfile[] = initiativeResult.results.map((row) => ({ id: row.id, title: row.title, status: row.status, priority: row.priority, owner: row.owner, targetDate: row.target_date, consequence: row.consequence, desiredOutcome: row.desired_outcome, decisionAsk: row.decision_ask, decisionQuestion: row.decision_question || row.decision_ask, asIsStatement: row.as_is_statement, toBeStatement: row.to_be_statement, successMeasures: row.success_measures, briefingAudience: row.briefing_audience, decisionNeededBy: row.decision_needed_by, problemStatement: row.problem_statement, driversConstraints: row.drivers_constraints, romHoursPerPoint: Number.isFinite(row.rom_hours_per_point) && Number(row.rom_hours_per_point) > 0 ? Number(row.rom_hours_per_point) : 500, romConversionRationale: row.rom_conversion_rationale, closedAt: row.closed_at, primaryReleaseId: row.primary_release_id, primaryReleaseName: row.primary_release_name, updatedAt: row.updated_at }));
  ...
  const objectives: IncumbentObjective[] = objectiveResult.results.map((row) => ({ id: row.id, changeRequestId: row.change_request_id, externalSystem: row.external_system, externalIdentifier: row.external_identifier, externalItemType: row.external_item_type || "Objective", title: row.title, summary: row.summary, sourceDescription: row.source_description ?? row.summary, governmentSynopsis: row.government_synopsis, descriptionAuthority: row.description_authority || "migrated_unclassified", technicalOwner: row.technical_owner, status: row.status, plannedStart: row.planned_start, plannedFinish: row.planned_finish, actualStart: row.actual_start, actualFinish: row.actual_finish, sourceLocator: row.source_locator, sourceAsOf: row.source_as_of, estimates: estimatesByObjective.get(row.id) || [], updatedAt: row.updated_at }));
  ...
  const solutionSteps: SolutionOptionStep[] = solutionStepResult.results.map((row) => ({ id: row.id, optionId: row.option_id, title: row.title, description: row.description, expectedResult: row.expected_result, parentStepId: row.parent_step_id, wbsCode: row.wbs_code, owner: row.owner, planningStart: row.planning_start, planningFinish: row.planning_finish, planningEffortHours: row.planning_effort_hours, planningEffortBasis: row.planning_effort_basis, sortOrder: row.sort_order, updatedAt: row.updated_at }));
  const solutionStepReferences: SolutionStepReference[] = solutionStepReferenceResult.results.map((row) => ({ id: row.id, optionId: row.option_id, stepId: row.step_id, referenceKind: row.reference_kind, sourceId: row.source_id, reference: row.reference, label: row.label, rationale: row.rationale, updatedAt: row.updated_at }));
  const solutionStepDependencies: SolutionStepDependency[] = solutionStepDependencyResult.results.map((row) => ({ id: row.id, optionId: row.option_id, predecessorStepId: row.predecessor_step_id, successorStepId: row.successor_step_id, relationship: row.relationship, lagDays: Number(row.lag_days || 0), rationale: row.rationale, updatedAt: row.updated_at }));
  ...
  const solutionKnockOns: SolutionKnockOn[] = solutionKnockOnResult.results.map((row) => ({ id: row.id, optionId: row.option_id, origin: "government", classification: row.classification, affectedKind: row.affected_kind, affectedId: row.affected_id, affectedReference: row.affected_reference, timing: row.timing, likelihood: row.likelihood, impact: row.impact, confidence: row.confidence, narrative: row.narrative, mitigation: row.mitigation, sourceAuthority: "Government analysis", sourceReference: row.affected_reference, updatedAt: row.updated_at }));
  const requestById = new Map(changes.requests.map((request) => [request.id, request]));
  const objectiveById = new Map(objectives.map((objective) => [objective.id, objective]));
  for (const option of solutionOptions) {
    const selectedRequestIds = new Set(solutionChangeRequestLinks.filter((link) => link.optionId === option.id).map((link) => link.changeRequestId));
    const selectedObjectiveIds = new Set(solutionObjectiveLinks.filter((link) => link.optionId === option.id).map((link) => link.objectiveId));
    for (const dependency of changes.dependencies.filter((item) => selectedRequestIds.has(item.predecessorRequestId) || selectedRequestIds.has(item.successorRequestId))) {
      const predecessor = requestById.get(dependency.predecessorRequestId);
      const successor = requestById.get(dependency.successorRequestId);
      solutionKnockOns.push({ id: `derived:cr-dependency:${option.id}:${dependency.id}`, optionId: option.id, origin: "derived", classification: "dependency", affectedKind: "change_request", affectedId: dependency.successorRequestId, affectedReference: successor?.externalIdentifier || null, timing: dependency.sourceAsOf, likelihood: "unassessed", impact: dependency.consequenceIfUnmet ? "high" : "unassessed", confidence: dependency.confidence === "confirmed" ? "high" : dependency.confidence === "assessed" ? "medium" : "low", narrative: `${predecessor?.externalIdentifier || "A prerequisite Change Request"} ${dependency.dependencyType} ${successor?.externalIdentifier || "a selected Change Request"}.${dependency.consequenceIfUnmet ? ` ${dependency.consequenceIfUnmet}` : ""}`, mitigation: null, sourceAuthority: "Change Request dependency record", sourceReference: dependency.sourceReference, updatedAt: dependency.sourceAsOf || option.updatedAt });
    }
    for (const effect of changes.effects.filter((item) => selectedRequestIds.has(item.changeRequestId) && Boolean(item.consequence))) {
      const request = requestById.get(effect.changeRequestId);
      solutionKnockOns.push({ id: `derived:change-effect:${option.id}:${effect.id}`, optionId: option.id, origin: "derived", classification: effect.action === "remove" ? "risk" : "second_order_effect", affectedKind: effect.subjectKind, affectedId: effect.subjectId, affectedReference: effect.subjectLabel, timing: effect.toReleaseName || effect.fromReleaseName, likelihood: "unassessed", impact: "unassessed", confidence: effect.confidence === "confirmed" ? "high" : effect.confidence === "assessed" ? "medium" : "low", narrative: effect.consequence!, mitigation: null, sourceAuthority: `${request?.externalIdentifier || "Change Request"} affected-object record`, sourceReference: request?.sourceLocator || null, updatedAt: request?.updatedAt || option.updatedAt });
    }
    for (const dependency of objectiveDependencies.filter((item) => selectedRequestIds.has(item.dependentChangeRequestId) || selectedObjectiveIds.has(item.prerequisiteObjectiveId))) {
      const objective = objectiveById.get(dependency.prerequisiteObjectiveId);
      solutionKnockOns.push({ id: `derived:objective-dependency:${option.id}:${dependency.id}`, optionId: option.id, origin: "derived", classification: "dependency", affectedKind: "objective", affectedId: dependency.prerequisiteObjectiveId, affectedReference: objective?.externalIdentifier || null, timing: dependency.sourceAsOf, likelihood: "unassessed", impact: "unassessed", confidence: "unassessed", narrative: dependency.rationale, mitigation: null, sourceAuthority: "Objective dependency record", sourceReference: dependency.sourceReference || dependency.evidenceReference, updatedAt: dependency.updatedAt });
    }
    for (const objectiveId of selectedObjectiveIds) {
      const gaps = requirements.filter((item) => item.objectiveId === objectiveId && !["verified", "not_applicable"].includes(item.traceStatus));
      if (!gaps.length) continue;
      const objective = objectiveById.get(objectiveId);
      solutionKnockOns.push({ id: `derived:acceptance-gap:${option.id}:${objectiveId}`, optionId: option.id, origin: "derived", classification: "constraint", affectedKind: "objective", affectedId: objectiveId, affectedReference: objective?.externalIdentifier || null, timing: objective?.plannedFinish || null, likelihood: "high", impact: "unassessed", confidence: "high", narrative: `${gaps.length} requirement trace${gaps.length === 1 ? " is" : "s are"} not yet verified or adjudicated not applicable.`, mitigation: "Complete requirement trace and acceptance planning before adjudication.", sourceAuthority: "Requirement and acceptance records", sourceReference: null, updatedAt: objective?.updatedAt || option.updatedAt });
    }
  }
...
export function bundleFor(workspace: Omit<InitiativeDecisionWorkspace, "assessments"> & { assessments?: InitiativeDecisionWorkspace["assessments"] }, initiativeId: string): InitiativeDecisionBundle {
  ...
  const solutionChangeRequestLinks = workspace.solutionChangeRequestLinks.filter((item) => solutionOptionIds.has(item.optionId));
  const solutionObjectiveLinks = workspace.solutionObjectiveLinks.filter((item) => solutionOptionIds.has(item.optionId));
  const requestIds = new Set(solutionChangeRequestLinks.map((item) => item.changeRequestId));
...
export async function saveDecisionProfile(db: Database, actor: Actor, body: Record<string, unknown>) {
  ...
  const romHoursPerPoint = body.romHoursPerPoint === undefined ? Number(before?.rom_hours_per_point || 500) : numberOrNull(body.romHoursPerPoint) ?? 500;
  ...
  const retained = (input: unknown, column: string) => input === undefined ? nullable(before?.[column]) : nullable(input);
  const next = { problemStatement: retained(body.problemStatement, "problem_statement"), driversConstraints: retained(body.driversConstraints, "drivers_constraints"), successMeasures: retained(body.successMeasures, "success_measures"), decisionNeededBy: retained(body.decisionNeededBy, "decision_needed_by"), decisionQuestion: retained(body.decisionQuestion, "decision_question"), desiredOutcome: retained(body.desiredOutcome, "desired_outcome"), owner: retained(body.owner, "owner"), romHoursPerPoint, romConversionRationale: retained(body.romConversionRationale, "rom_conversion_rationale") };
  const at = now();
  await db.batch([
    db.prepare("UPDATE initiative SET problem_statement=?,drivers_constraints=?,success_measures=?,decision_needed_by=?,decision_question=?,desired_outcome=?,owner=?,rom_hours_per_point=?,rom_conversion_rationale=?,updated_at=? WHERE id=? AND program_id=?").bind(next.problemStatement, next.driversConstraints, next.successMeasures, next.decisionNeededBy, next.decisionQuestion, next.desiredOutcome, next.owner, next.romHoursPerPoint, next.romConversionRationale, at, initiativeId, PROGRAM_ID),
...
export async function saveObjective(db: Database, actor: Actor, body: Record<string, unknown>) {
  ...
  const statements: D1PreparedStatement[] = [
    db.prepare("INSERT INTO incumbent_objective (id,program_id,change_request_id,external_system,external_identifier,external_item_type,title,summary,source_description,government_synopsis,description_authority,technical_owner,status,planned_start,planned_finish,actual_start,actual_finish,source_locator,source_as_of,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET change_request_id=excluded.change_request_id,external_system=excluded.external_system,external_identifier=excluded.external_identifier,external_item_type=excluded.external_item_type,title=excluded.title,summary=excluded.summary,source_description=excluded.source_description,government_synopsis=excluded.government_synopsis,description_authority=excluded.description_authority,technical_owner=excluded.technical_owner,status=excluded.status,planned_start=excluded.planned_start,planned_finish=excluded.planned_finish,actual_start=excluded.actual_start,actual_finish=excluded.actual_finish,source_locator=excluded.source_locator,source_as_of=excluded.source_as_of,updated_at=excluded.updated_at").bind(objectiveId, PROGRAM_ID, changeRequestId, externalSystem, externalIdentifier, externalItemType, title, nullable(body.sourceDescription ?? body.summary), nullable(body.sourceDescription ?? body.summary), nullable(body.governmentSynopsis), ["reported","analyst_transcribed","migrated_unclassified"].includes(clean(body.descriptionAuthority)) ? clean(body.descriptionAuthority) : "analyst_transcribed", nullable(body.technicalOwner), status, plannedStart, plannedFinish, actualStart, actualFinish, nullable(body.sourceLocator), nullable(body.sourceAsOf), actor.id, at, at),
