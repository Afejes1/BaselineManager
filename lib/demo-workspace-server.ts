...
export async function enrichDemonstrationWorkspace(db: Database, actor: Actor) {
  ...
  statements.push(db.prepare("UPDATE initiative SET problem_statement=?,drivers_constraints=?,updated_at=? WHERE id=?")
    .bind("Two in-scope mission applications depend on an unsupported Java 8 runtime. The current baseline cannot demonstrate a complete supported-runtime or SBOM position, creating cyber, sustainment, and fielding uncertainty.", "Release 7 fielding window; uninterrupted mission behavior; approved supported runtime; air-gapped deployment; Tier 3 mission and Tier 4 system acceptance; reversible fielding path. These are known boundaries, not probabilistic risks.", at, javaInitiativeId));
  ...
  const solutionOptions = [
    ["demo-solution-java8-status-quo", "Continue sustainment and accept unsupported-runtime exposure", "continue sustainment and accept unsupported-runtime exposure", "status_quo", "under_review", "Maintain the current applications and operating procedures while monitoring support exceptions and cyber findings.", "Mission capability remains available in the near term, but the shared outcome of eliminating Java 8 is not achieved.", "Avoids near-term transformation disruption and cost; continues exception handling, duplicated sustainment work, and future fielding constraints.", "Exploit exposure, loss of vendor support, unplanned remediation, and schedule compression remain accepted or grow over time.", "Assumes the authorizing authority will continue to accept the unsupported-runtime posture and sustainment resources remain available.", 0],
    ["demo-solution-java8-targeted", "Targeted supported-runtime upgrade", "targeted supported-runtime upgrade", "candidate", "recommended", "Inventory the fielded runtime, upgrade the two retained applications and their build/deployment chain, then execute mission, system, cyber, configuration, and rollback acceptance.", "Eliminates Java 8 from the two in-scope applications while preserving their current functional design and accepted mission behavior.", "Bounds the change to the supported-runtime transition, but still consumes integration range, operator, requirements, and acceptance capacity.", "Undocumented native dependencies or interface behavior may expand remediation; regression or rollback failure may delay fielding.", "Assumes the inventory remains complete, both applications can run on the approved LTS without architectural redesign, and the test range is available on the planned dates.", 1],
    ["demo-solution-java8-modernize", "Modernize priority application components and containerize the remainder", "modernize priority application components and containerize the remainder", "candidate", "under_review", "Use the runtime transition to redesign the highest-risk application components, remove functions no longer needed, and package the retained workloads in supported containers.", "Eliminates Java 8 and reduces selected long-term maintainability and deployment risks beyond a stack-only upgrade.", "May reduce future sustainment burden, but expands design, interface, training, migration, and acceptance work beyond the currently sourced upgrade Objective.", "Broader behavior change, stakeholder disruption, requirements churn, and incomplete estimate coverage may outweigh the lifecycle benefit.", "The redesign boundary, retirement candidates, container platform capacity, and a source-backed modernization Objective/ROM have not yet been established.", 2],
  ] as const;
  ...
  const solutionSteps = [
    ["demo-solution-java8-status-quo", "Maintain current approved exception", "Document the unsupported-runtime posture and retain the existing fielded baseline.", "Current mission behavior remains unchanged; the modernization outcome remains unmet."],
    ["demo-solution-java8-status-quo", "Monitor support and cyber exposure", "Review vendor support, scan findings, incidents, and sustainment burden at each decision cadence.", "The accepting authority receives current exposure evidence and a trigger for reconsideration."],
    ["demo-solution-java8-targeted", "Reconcile runtime and SBOM inventory", "Accept the exact product, installation, runtime, and transitive dependency population.", "Upgrade scope and estimate coverage are bounded."],
    ["demo-solution-java8-targeted", "Upgrade runtime and build chain", "Move the retained applications, libraries, base images, and deployment automation to the approved LTS.", "An integration-ready supported-runtime build exists with rollback artifacts."],
    ["demo-solution-java8-targeted", "Remediate interfaces and regressions", "Resolve compatibility findings against reconciled requirements.", "The build meets its system verification entry criteria."],
    ["demo-solution-java8-targeted", "Verify and field", "Execute Tier 3/Tier 4 evidence, cyber scan, SBOM reconciliation, and rollback rehearsal.", "Named authorities can accept or reject fielding using governed evidence."],
    ["demo-solution-java8-modernize", "Bound functions and stakeholder outcomes", "Identify functions to retain, redesign, replace, or retire and validate mission/operator priorities.", "A Government-approved modernization boundary exists."],
    ["demo-solution-java8-modernize", "Create a source-backed modernization Objective", "Require the incumbent to enumerate redesigned components, interfaces, steps, dates, ROM, and acceptance changes.", "The broader alternative becomes quantitatively comparable without inventing effort."],
    ["demo-solution-java8-modernize", "Prototype and adjudicate", "Demonstrate the highest-risk component and compare validated outcomes with the targeted upgrade.", "Government can decide whether added lifecycle benefit justifies the expanded change."],
  ] as const;
  ...
  for (const [index, step] of solutionSteps.entries()) {
    ...
    statements.push(db.prepare("INSERT INTO solution_option_step (id,option_id,title,description,expected_result,sort_order,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)").bind(`demo-solution-step-${index + 1}`, ...step, sortOrder, actor.id, at, at));
  }
