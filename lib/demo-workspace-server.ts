...
export async function enrichDemonstrationWorkspace(db: Database, actor: Actor) {
  ...
  ].entries()) statements.push(db.prepare("INSERT INTO work_package (id,initiative_id,change_request_id,objective_id,parent_id,wbs_code,title,owner,planned_start,due_date,status,definition_of_done,progress_basis,notes,sort_order,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(`demo-java-wbs-${index + 1}`, javaInitiativeId, work[2], work[1], null, work[0], work[3], work[4], work[5], work[6], work[7], work[9], "Status is supported by the linked estimate, requirement, or briefing artifact.", work[8], index, at, at));
  for (const dependency of [
