...
export async function enrichDemonstrationWorkspace(db: Database, actor: Actor) {
  ...
  const javaObjectiveIds = ["demo-objective-java-inventory", "demo-objective-java-upgrade", "demo-objective-java-acceptance", "demo-objective-java-modernization"];
  // Solution alternatives are ordinary governed rows in the synthetic
  // workspace. Clear them before recreating Objectives so repeated demo loads
  // preserve FK order and never leave stale analytical selections behind.
  const solutionResetOperationId = `demo-solution-reset:${crypto.randomUUID()}`;
  statements.push(db.prepare("INSERT INTO initiative_solution_decision_maintenance_lock (id,operation_id,created_at) VALUES (1,?,?)").bind(solutionResetOperationId, at));
  ...
  statements.push(db.prepare("DELETE FROM solution_option_assessment WHERE option_id IN (SELECT id FROM solution_option WHERE initiative_id=?)").bind(javaInitiativeId));
  statements.push(db.prepare("DELETE FROM solution_option_objective WHERE option_id IN (SELECT id FROM solution_option WHERE initiative_id=?)").bind(javaInitiativeId));
  statements.push(db.prepare("DELETE FROM solution_option_change_request WHERE option_id IN (SELECT id FROM solution_option WHERE initiative_id=?)").bind(javaInitiativeId));
  statements.push(db.prepare("DELETE FROM solution_option_step WHERE option_id IN (SELECT id FROM solution_option WHERE initiative_id=?)").bind(javaInitiativeId));
  statements.push(db.prepare("DELETE FROM solution_option WHERE initiative_id=?").bind(javaInitiativeId));
  statements.push(db.prepare("DELETE FROM initiative_solution_decision_maintenance_lock WHERE id=1 AND operation_id=?").bind(solutionResetOperationId));
  ...
  statements.push(db.prepare("INSERT INTO initiative (id,program_id,primary_release_id,title,normalized_title,status,priority,owner,target_date,consequence,desired_outcome,decision_ask,as_is_statement,to_be_statement,success_measures,briefing_audience,decision_needed_by,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET primary_release_id=excluded.primary_release_id,title=excluded.title,normalized_title=excluded.normalized_title,status=excluded.status,priority=excluded.priority,owner=excluded.owner,target_date=excluded.target_date,consequence=excluded.consequence,desired_outcome=excluded.desired_outcome,decision_ask=excluded.decision_ask,as_is_statement=excluded.as_is_statement,to_be_statement=excluded.to_be_statement,success_measures=excluded.success_measures,briefing_audience=excluded.briefing_audience,decision_needed_by=excluded.decision_needed_by,updated_at=excluded.updated_at")
    .bind(javaInitiativeId, PROGRAM_ID, null, "Reduce PMA downtime and cyber exposure", "reduce pma downtime and cyber exposure", "active", "high", "Government Mission Systems Architecture (synthetic)", null, null, "Reduce avoidable PMA service interruption and cyber exposure while preserving accepted mission behavior and a reversible fielding path.", null, null, null, "No executable Java 8 runtime in the selected workload boundary; signed SBOM coverage; accepted mission and system evidence; rollback demonstrated; service-interruption exposure explicitly compared for every alternative.", null, "2026-09-15", actor.id, at, at));
  statements.push(db.prepare("UPDATE initiative SET problem_statement=?,drivers_constraints=?,decision_question=?,rom_hours_per_point=?,rom_conversion_rationale=?,updated_at=? WHERE id=?")
    .bind("PMA sustainment carries avoidable downtime and cyber exposure because two mission applications depend on an unsupported runtime and the current baseline cannot demonstrate a complete supported-runtime or SBOM position.", "Preserve mission behavior; use the approved container/runtime boundary; operate without outside calls; retain a reversible fielding path; keep incumbent source dates distinct from Government planning overlays; require Tier 3 mission and Tier 4 system acceptance.", "Which bounded technical alternative should the Government direct to reduce PMA downtime and cyber exposure without accepting unjustified scope, schedule, or mission risk?", 500, "Synthetic demonstration assumption: one Lockheed ROM point equals 500 planning hours unless the Initiative records a different conversion and rationale.", at, javaInitiativeId));
  const objectivePlan = [
    ...
    { id: javaObjectiveIds[3], requestId: "demo-change-java-upgrade", externalId: "DEMO-OBJ-J8-04", title: "Modernize priority components and containerize retained workloads", summary: "Define the redesign and retirement boundary, modernize the selected components, and package retained workloads on the approved container platform. Scope, ROM, and dates are intentionally unsourced pending incumbent decomposition.", owner: "Incumbent Mission Applications IPT (synthetic)", status: "proposed", start: null, finish: null, actualStart: null, actualFinish: null, source: "DEMO://OBJECTIVES/J8-04" },
  ] as const;
  ...
  // Worked Solution Engineering case.  No totals are stored here: the UI
  // derives every estimate and date from the explicitly selected Objectives.
  const solutionOptions = [
    ["demo-solution-java8-status-quo", "Continue PMA sustainment as-is", "continue pma sustainment as is", "status_quo", "under_review", "Maintain the current applications and operating procedures while monitoring service interruptions, support exceptions, and cyber findings.", "Near-term mission behavior remains unchanged, but downtime and unsupported-runtime exposure are not materially reduced.", "Avoids near-term transformation disruption while continuing exception handling and future fielding constraints.", "Vendor support, exploit, unplanned-remediation, and service-interruption exposure remain accepted or grow over time.", "Assumes the authorizing authority will continue to accept the unsupported-runtime posture and sustainment resources remain available.", 0],
    ["demo-solution-java8-targeted", "Containerize and rehost retained PMA workloads", "containerize and rehost retained pma workloads", "candidate", "recommended", "Inventory the fielded runtime, rehost the two retained applications in supported containers, upgrade their runtime/build chain, and execute mission, system, cyber, configuration, and rollback acceptance.", "Reduces unsupported-runtime and deployment-variance exposure while preserving the current functional design.", "Bounds behavior change and creates a repeatable deployment unit, but still consumes integration range, operator, requirements, and acceptance capacity.", "Undocumented native dependencies or interface behavior may expand remediation; container-host constraints or rollback failure may delay fielding.", "Assumes both applications can run in the approved container host without architectural redesign and the test range is available on the Government planning dates.", 1],
    ["demo-solution-java8-modernize", "Selectively modernize priority PMA components", "selectively modernize priority pma components", "candidate", "under_review", "Retire functions no longer needed, redesign only the highest-risk components, and containerize the retained workloads on the approved platform.", "Reduces downtime and cyber exposure while also addressing selected long-term maintainability risks beyond rehosting.", "May reduce future sustainment burden, but expands design, interface, training, migration, and acceptance work beyond a stack-only transition.", "Broader behavior change, stakeholder disruption, requirements churn, and incomplete estimate coverage may outweigh the lifecycle benefit.", "The redesign boundary, retirement candidates, container-platform capacity, and a source-backed modernization ROM must be validated before selection.", 2],
  ] as const;
  for (const option of solutionOptions) statements.push(db.prepare("INSERT INTO solution_option (id,initiative_id,title,normalized_title,option_type,status,summary,projected_outcome,expected_consequences,residual_risks,assumptions,sort_order,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(option[0], javaInitiativeId, ...option.slice(1), actor.id, at, at));
  const solutionChangeLinks = [
    ["demo-solution-java8-targeted", "demo-change-java-inventory", "enables", "Creates the authoritative scope and estimate boundary."],
    ["demo-solution-java8-targeted", "demo-change-java-upgrade", "delivers", "Performs the bounded runtime, library, build, and deployment transition."],
    ["demo-solution-java8-targeted", "demo-change-java-acceptance", "delivers", "Produces the acceptance evidence required before fielding."],
    ["demo-solution-java8-modernize", "demo-change-java-inventory", "enables", "Provides the current-state inventory used to choose modernization boundaries."],
    ["demo-solution-java8-modernize", "demo-change-java-upgrade", "supports", "Contains reusable runtime-transition work, but does not yet describe the broader redesign."],
    ["demo-solution-java8-modernize", "demo-change-java-acceptance", "supports", "Provides a reusable acceptance frame that must be expanded for changed behavior."],
  ] as const;
  for (const [index, link] of solutionChangeLinks.entries()) statements.push(db.prepare("INSERT INTO solution_option_change_request (id,option_id,change_request_id,relationship,rationale,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)").bind(`demo-solution-change-${index + 1}`, ...link, actor.id, at, at));
  const solutionObjectiveLinks = [
    ["demo-solution-java8-targeted", javaObjectiveIds[0], "enabling", "Inventory is required to bound the fielded population and source estimate."],
    ["demo-solution-java8-targeted", javaObjectiveIds[1], "required", "This is the incumbent delivery Objective for the targeted runtime transition."],
    ["demo-solution-java8-targeted", javaObjectiveIds[2], "required", "Acceptance is required to demonstrate the shared outcome before fielding."],
    ["demo-solution-java8-modernize", javaObjectiveIds[0], "enabling", "Inventory informs component retention and retirement analysis."],
    ["demo-solution-java8-modernize", javaObjectiveIds[1], "optional", "The current upgrade Objective is shown as an add-on because it does not yet bound application redesign."],
    ["demo-solution-java8-modernize", javaObjectiveIds[2], "required", "The existing acceptance Objective is a minimum starting point and will require expansion."],
    ["demo-solution-java8-modernize", javaObjectiveIds[3], "required", "This is the unsourced modernization work that must be decomposed and estimated before comparison can be complete."],
  ] as const;
  for (const [index, link] of solutionObjectiveLinks.entries()) statements.push(db.prepare("INSERT INTO solution_option_objective (id,option_id,objective_id,role,rationale,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)").bind(`demo-solution-objective-${index + 1}`, ...link, actor.id, at, at));
  const solutionSteps = [
    ["demo-solution-java8-status-quo", "Maintain current approved exception", "Document the unsupported-runtime posture and retain the existing fielded baseline.", "Current mission behavior remains unchanged; the shared outcome remains unmet.", null, "SQ-1", "Government cyber authority", null, null, null, "Continuing sustainment effort is not source-estimated."],
    ["demo-solution-java8-status-quo", "Monitor downtime and cyber exposure", "Review service interruptions, support status, scan findings, incidents, and sustainment burden at each decision cadence.", "The accepting authority receives current exposure evidence and an explicit trigger for reconsideration.", null, "SQ-2", "Government sustainment lead", null, null, null, "No dates inferred for continuing operations."],
    ["demo-solution-java8-targeted", "Reconcile runtime and SBOM inventory", "Accept the exact product, installation, runtime, dependency, and container-host population.", "Rehost scope and estimate coverage are bounded.", null, "CR-1", "Government configuration lead", "2026-09-21", "2026-10-09", 320, "Government planning overlay for source reconciliation."],
    ["demo-solution-java8-targeted", "Containerize and rehost retained workloads", "Move the retained applications, libraries, base images, and deployment automation to the approved container host and supported LTS.", "An integration-ready supported container build exists with rollback artifacts.", null, "CR-2", "Mission Applications IPT", "2026-10-12", "2027-01-29", null, "Effort is derived from selected source Objectives, not duplicated on the WBS row."],
    ["demo-solution-java8-targeted", "Remediate interfaces and regressions", "Resolve compatibility findings against reconciled requirements.", "The containerized build meets system verification entry criteria.", "demo-solution-step-4", "CR-2.1", "Integration team", "2026-12-01", "2027-02-26", 1200, "Government planning allowance; kept separate from incumbent ROM."],
    ["demo-solution-java8-targeted", "Verify and field", "Execute Tier 3/Tier 4 evidence, cyber scan, SBOM reconciliation, and rollback rehearsal.", "Named authorities can accept or reject fielding using governed evidence.", null, "CR-3", "Government test and fielding authority", "2027-03-01", "2027-05-28", null, "Dates are a Government overlay; Objective source dates remain visible separately."],
    ["demo-solution-java8-modernize", "Bound functions and stakeholder outcomes", "Identify functions to retain, redesign, replace, or retire and validate mission/operator priorities.", "A Government-approved modernization boundary exists.", null, "MOD-1", "Government solution architect", "2026-09-21", "2026-11-06", 640, "Time-boxed Government analysis overlay."],
    ["demo-solution-java8-modernize", "Obtain source-backed modernization decomposition", "Require the incumbent to enumerate redesigned components, interfaces, steps, dates, ROM, and acceptance changes.", "The broader alternative becomes quantitatively comparable without inventing effort.", null, "MOD-2", "Incumbent Mission Applications IPT", null, null, null, "Unscheduled until a source-backed decomposition is received."],
    ["demo-solution-java8-modernize", "Prototype and adjudicate", "Demonstrate the highest-risk component and compare validated outcomes with the rehost alternative.", "Government can decide whether added lifecycle benefit justifies the expanded change.", null, "MOD-3", "Government architecture and test leads", null, null, null, "No dates inferred pending MOD-2."],
  ] as const;
  const stepOrder = new Map<string, number>();
  for (const [index, step] of solutionSteps.entries()) {
    const sortOrder = stepOrder.get(step[0]) || 0;
    stepOrder.set(step[0], sortOrder + 1);
    statements.push(db.prepare("INSERT INTO solution_option_step (id,option_id,title,description,expected_result,parent_step_id,wbs_code,owner,planning_start,planning_finish,planning_effort_hours,planning_effort_basis,sort_order,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(`demo-solution-step-${index + 1}`, ...step, sortOrder, actor.id, at, at));
  }
  for (const [index, dependency] of [
    ["demo-solution-java8-targeted", "demo-solution-step-3", "demo-solution-step-4", "FS", 0, "The reconciled runtime and SBOM inventory bounds the containerization work."],
    ["demo-solution-java8-targeted", "demo-solution-step-4", "demo-solution-step-5", "SS", 35, "Interface remediation begins after the rehost build has produced its first integration candidate."],
    ["demo-solution-java8-targeted", "demo-solution-step-5", "demo-solution-step-6", "FF", 0, "Regression remediation and verification must finish together before fielding recommendation."],
    ["demo-solution-java8-modernize", "demo-solution-step-7", "demo-solution-step-8", "FS", 0, "The Government boundary must be accepted before requesting a sourced modernization decomposition."],
    ["demo-solution-java8-modernize", "demo-solution-step-8", "demo-solution-step-9", "FS", 0, "The prototype remains unscheduled until source scope and ROM are available."],
  ] as const) statements.push(db.prepare("INSERT INTO solution_step_dependency (id,option_id,predecessor_step_id,successor_step_id,relationship,lag_days,rationale,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(`demo-solution-dependency-${index + 1}`, ...dependency, actor.id, at, at));
  for (const [index, reference] of [
    ["demo-solution-java8-targeted", "demo-solution-step-3", "change_request", "demo-change-java-inventory", null, "DEMO-MCP inventory scope", "Bounds the option before work begins."],
    ["demo-solution-java8-targeted", "demo-solution-step-4", "objective", javaObjectiveIds[1], null, "DEMO-OBJ-J8-02", "Source Objective and ROM for rehost delivery."],
    ["demo-solution-java8-targeted", "demo-solution-step-6", "objective", javaObjectiveIds[2], null, "DEMO-OBJ-J8-03", "Source acceptance work and dates."],
    ["demo-solution-java8-modernize", "demo-solution-step-8", "objective", javaObjectiveIds[3], null, "DEMO-OBJ-J8-04", "Unscheduled source Objective exposes the current analysis gap."],
  ] as const) statements.push(db.prepare("INSERT INTO solution_step_reference (id,option_id,step_id,reference_kind,source_id,reference,label,rationale,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)").bind(`demo-solution-reference-${index + 1}`, ...reference, actor.id, at, at));
  for (const [index, knockOn] of [
    ["demo-solution-java8-status-quo", "risk", "PMA mission services", "current fielded baseline", "Lifecycle", "high", "high", "high", "Continued unsupported-runtime exceptions increase unplanned remediation and outage exposure as vendor support recedes.", "Define an explicit reconsideration trigger tied to support or incident evidence."],
    ["demo-solution-java8-targeted", "benefit", "PMA deployment process", "approved container host", "Fielding and sustainment", "medium", "high", "medium", "Repeatable container packaging reduces deployment variance and may shorten restoration after a failed fielding event.", "Exercise rollback and restoration during Tier 4 verification."],
    ["demo-solution-java8-targeted", "constraint", "Integration range", "Tier 3/Tier 4 acceptance window", "2027 acceptance window", "high", "high", "high", "The rehost alternative consumes a bounded integration and acceptance window even when application behavior is intended to remain unchanged.", "Name acceptance authorities and reserve the range before direction."],
    ["demo-solution-java8-modernize", "second_order_effect", "Operators and maintainers", "training and procedure baseline", "Before fielding", "medium", "medium", "low", "Selective modernization may alter workflows and sustainment procedures beyond the currently sourced runtime transition.", "Validate the retained-function boundary with operators before prototyping."],
  ] as const) statements.push(db.prepare("INSERT INTO solution_option_knock_on (id,option_id,classification,affected_kind,affected_reference,timing,likelihood,impact,confidence,narrative,mitigation,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(`demo-solution-knock-on-${index + 1}`, ...knockOn, actor.id, at, at));
  const solutionAssessments = [
    ["demo-solution-java8-status-quo", "outcome_alignment", "unfavorable", "Does not achieve the shared supported-runtime outcome.", "DEMO://SOLUTION-ENGINEERING/JAVA8/STATUS-QUO", "high"],
    ["demo-solution-java8-status-quo", "delivery_effort", "favorable", "Requires no sourced transformation work; continuing sustainment effort is not estimated by this option.", "DEMO://SOLUTION-ENGINEERING/JAVA8/STATUS-QUO", "medium"],
    ["demo-solution-java8-status-quo", "cyber_lifecycle", "unfavorable", "Continues unsupported-runtime cyber and vendor-support exposure.", "DEMO://SOLUTION-ENGINEERING/JAVA8/STATUS-QUO", "high"],
    ["demo-solution-java8-targeted", "outcome_alignment", "favorable", "Directly removes the named Java 8 dependency from the two in-scope applications.", "DEMO://SOLUTION-ENGINEERING/JAVA8/TARGETED", "high"],
    ["demo-solution-java8-targeted", "delivery_effort", "mixed", "All three selected Objectives have incumbent estimates; the Government estimate covers only two of three.", "DEMO://SOLUTION-ENGINEERING/JAVA8/TARGETED", "high"],
    ["demo-solution-java8-targeted", "requirements_acceptance", "mixed", "Acceptance work is explicit, but the interface requirement remains unresolved.", "DEMO://SOLUTION-ENGINEERING/JAVA8/TARGETED", "medium"],
    ["demo-solution-java8-modernize", "outcome_alignment", "favorable", "Could achieve the shared outcome and improve lifecycle posture beyond a stack-only transition.", "DEMO://SOLUTION-ENGINEERING/JAVA8/MODERNIZE", "low"],
    ["demo-solution-java8-modernize", "delivery_effort", "unassessed", null, "DEMO://SOLUTION-ENGINEERING/JAVA8/MODERNIZE", "unassessed"],
    ["demo-solution-java8-modernize", "stakeholder_impact", "mixed", "May simplify future support but requires broader operator, requirements, training, and product-team engagement.", "DEMO://SOLUTION-ENGINEERING/JAVA8/MODERNIZE", "low"],
  ] as const;
  for (const [index, assessment] of solutionAssessments.entries()) statements.push(db.prepare("INSERT INTO solution_option_assessment (id,option_id,criterion,rating,narrative,source_reference,confidence,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(`demo-solution-assessment-${index + 1}`, ...assessment, actor.id, at, at));
  statements.push(db.prepare("INSERT INTO initiative_solution_decision (id,initiative_id,selected_option_id,disposition,decision_authority,decision_date,rationale,accepted_residual_risk,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
    .bind("demo-solution-java8-decision", javaInitiativeId, null, "pending", null, null, null, null, actor.id, at, at));
  ...
  for (const [index, work] of ([] as unknown[][]).entries()) {
  ...
  for (const dependency of [] as readonly (readonly unknown[])[]) statements.push(db.prepare("INSERT INTO work_package_dependency (id,predecessor_work_package_id,successor_work_package_id,relationship,lag_days,status,rationale,source_reference,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)").bind(...dependency, actor.id, at, at));

  statements.push(audit(db, actor, "demonstration_workspace_enriched", "baseline_workspace", WORKSPACE_ID, { occurrences: rows.length, governedInfrastructure: true, infrastructureNodes: infrastructureNodes.length, releaseConfigurations: releaseConfigurations.length, managedTopology: true, rationaleRecords: rationaleRecordCount, platforms: platformPlan.length, changeRequests: changePlan.length, initiatives: 1, objectives: objectivePlan.length, requirements: requirementPlan.length, acceptanceCriteria: criterionPlan.length, solutionEngineeringCase: "Reduce PMA downtime and cyber exposure" }));
