...
export async function enrichDemonstrationWorkspace(db: Database, actor: Actor) {
  ...
  const effectPlan = [
    ...
    ["demo-change-java-inventory", "product", rows.find((row) => row.sourceKey === "DEMO-R6-001")?.productId, "assess", "runtime and SBOM evidence", r6, r6, "Java family reported; runtime version not governed in the 24-column source", "Installation-level Java runtime and signed SBOM inventory", "Bounds the affected product and installation population before funding the upgrade."],
    ["demo-change-java-inventory", "product", rows.find((row) => row.sourceKey === "DEMO-R6-002")?.productId, "assess", "runtime and SBOM evidence", r6, r6, "Java family reported; runtime version not governed in the 24-column source", "Installation-level Java runtime and signed SBOM inventory", "Makes the Java 8 claim inspectable without altering the retained spreadsheet contract."],
