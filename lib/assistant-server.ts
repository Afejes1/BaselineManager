...
type ObjectiveRow = { id: string; change_request_id: string | null; external_system: string; external_identifier: string; external_item_type: string; title: string; summary: string | null; source_description: string | null; government_synopsis: string | null; description_authority: string; technical_owner: string | null; status: string; planned_start: string | null; planned_finish: string | null; actual_start: string | null; actual_finish: string | null; source_locator: string | null; source_as_of: string | null };
...
export async function applyAssistantProposal(db: Database, actor: Actor, contextValue: unknown, proposalValue: unknown, fingerprintValue: unknown) {
  ...
  if (proposal.kind === "create_initiative") {
    ...
    const objectiveId = asText(fields.id);
    const current = objectiveId ? await db.prepare("SELECT id,change_request_id,external_system,external_identifier,external_item_type,title,summary,source_description,government_synopsis,description_authority,technical_owner,status,planned_start,planned_finish,actual_start,actual_finish,source_locator,source_as_of FROM incumbent_objective WHERE id=? AND program_id=?").bind(objectiveId, PROGRAM_ID).first<ObjectiveRow>() : null;
    ...
    const merged = { id: current?.id || "", changeRequestId, externalSystem: current?.external_system || "", externalIdentifier: current?.external_identifier || "", externalItemType: current?.external_item_type || "Objective", title: current?.title || "", summary: current?.summary || "", sourceDescription: current?.source_description || current?.summary || "", governmentSynopsis: current?.government_synopsis || "", descriptionAuthority: current?.description_authority || "migrated_unclassified", technicalOwner: current?.technical_owner || "", status: current?.status || "proposed", plannedStart: current?.planned_start || "", plannedFinish: current?.planned_finish || "", actualStart: current?.actual_start || "", actualFinish: current?.actual_finish || "", sourceLocator: current?.source_locator || "", sourceAsOf: current?.source_as_of || "", ...assistantFields(fields, assistantActionFieldNames(proposal.kind)) };
