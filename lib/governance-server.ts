...
import { informationOrigins, informationStatusSummary, type InformationOrigin } from "./information-status";
...
const informationOriginSet = new Set<InformationOrigin>(informationOrigins);
...
type RecordRow = { id: string; record_type: GovernanceRecordType; information_origin: InformationOrigin; external_reference: string | null; title: string; status: GovernanceRecordStatus; owner: string | null; occurred_at: string | null; participants: string | null; due_date: string | null; summary: string | null; decision_ask: string | null; action_items: string | null; impact: string | null; adjudication_authority: string | null; adjudicated_at: string | null; adjudication_rationale: string | null; created_at: string; updated_at: string };
...
export async function createGovernanceRecord(db: Database, actor: Actor, body: Record<string, unknown>) {
  ...
  const informationOrigin = informationOriginSet.has(body.informationOrigin as InformationOrigin) ? body.informationOrigin as InformationOrigin : "unclassified";
  const adjudicationAuthority = nullable(body.adjudicationAuthority);
  const adjudicatedAt = nullable(body.adjudicatedAt);
  const adjudicationRationale = nullable(body.adjudicationRationale);
  if (type === "decision" && status === "approved" && (!adjudicationAuthority || !adjudicatedAt || !adjudicationRationale)) throw new Error("An approved decision record requires a decision authority, decision date, and rationale.");
  const recordId = id("record");
  const at = now();
  const links = governanceLinks(body.links);
  await assertGovernanceLinkTargets(db, links);
  const statements: D1PreparedStatement[] = [
    db.prepare("INSERT INTO governance_record (id,program_id,record_type,information_origin,external_reference,title,status,owner,occurred_at,participants,due_date,summary,decision_ask,action_items,impact,adjudication_authority,adjudicated_at,adjudication_rationale,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(recordId, PROGRAM_ID, type, informationOrigin, nullable(body.externalReference), title, status, nullable(body.owner), nullable(body.occurredAt), nullable(body.participants), nullable(body.dueDate), nullable(body.summary), nullable(body.decisionAsk), nullable(body.actionItems), nullable(body.impact), adjudicationAuthority, adjudicatedAt, adjudicationRationale, actor.id, at, at),
  ];
  ...
  statements.push(audit(db, actor, "governance_record_created", "governance_record", recordId, { type, title, status, informationOrigin, adjudicationAuthority, adjudicatedAt, links }));
  await db.batch(statements);
  return recordId;
}
...
export async function updateGovernanceRecord(db: Database, actor: Actor, body: Record<string, unknown>) {
  ...
  const next = { recordType: type, informationOrigin: body.informationOrigin === undefined ? current.information_origin : informationOriginSet.has(body.informationOrigin as InformationOrigin) ? body.informationOrigin as InformationOrigin : "unclassified", externalReference: body.externalReference === undefined ? current.external_reference : nullable(body.externalReference), title: clean(body.title) || current.title, status, owner: body.owner === undefined ? current.owner : nullable(body.owner), occurredAt: body.occurredAt === undefined ? current.occurred_at : nullable(body.occurredAt), participants: body.participants === undefined ? current.participants : nullable(body.participants), dueDate: body.dueDate === undefined ? current.due_date : nullable(body.dueDate), summary: body.summary === undefined ? current.summary : nullable(body.summary), decisionAsk: body.decisionAsk === undefined ? current.decision_ask : nullable(body.decisionAsk), actionItems: body.actionItems === undefined ? current.action_items : nullable(body.action_items), impact: body.impact === undefined ? current.impact : nullable(body.impact), adjudicationAuthority: body.adjudicationAuthority === undefined ? current.adjudication_authority : nullable(body.adjudicationAuthority), adjudicatedAt: body.adjudicatedAt === undefined ? current.adjudicated_at : nullable(body.adjudicatedAt), adjudicationRationale: body.adjudicationRationale === undefined ? current.adjudication_rationale : nullable(body.adjudicationRationale) };
  if (next.recordType === "decision" && next.status === "approved" && (!next.adjudicationAuthority || !next.adjudicatedAt || !next.adjudicationRationale)) throw new Error("An approved decision record requires a decision authority, decision date, and rationale.");
  const statements: D1PreparedStatement[] = [
    db.prepare("UPDATE governance_record SET record_type=?,information_origin=?,external_reference=?,title=?,status=?,owner=?,occurred_at=?,participants=?,due_date=?,summary=?,decision_ask=?,action_items=?,impact=?,adjudication_authority=?,adjudicated_at=?,adjudication_rationale=?,updated_at=? WHERE id=?").bind(next.recordType, next.informationOrigin, next.externalReference, next.title, next.status, next.owner, next.occurredAt, next.participants, next.dueDate, next.summary, next.decisionAsk, next.actionItems, next.impact, next.adjudicationAuthority, next.adjudicatedAt, next.adjudicationRationale, now(), recordId),
  ];
...
export async function createExecutiveBrief(db: Database, actor: Actor, body: Record<string, unknown>, markdownFactory?: (context: { title: string; snapshot: BriefSnapshot }) => string, guard?: ExecutiveBriefCreationGuard) {
  ...
  const linkedRecords = await db.prepare("SELECT g.record_type,g.title,g.status,g.information_origin,g.adjudication_authority,g.adjudicated_at FROM governance_record g JOIN governance_record_link l ON l.governance_record_id=g.id WHERE l.entity_kind='initiative' AND l.entity_id=? ORDER BY g.updated_at DESC").bind(initiativeId).all<{ record_type: string; title: string; status: string; information_origin: InformationOrigin; adjudication_authority: string | null; adjudicated_at: string | null }>();
  ...
  const releaseContextNames = new Set(effectScope.results.flatMap((row) => [row.from_release_name, row.to_release_name, row.requested_release_name]).filter((name): name is string => Boolean(name)));
  const snapshot: BriefSnapshot = { asOf: now(), handlingMarking: PROGRAM_HANDLING_MARKING, releaseName: releaseContextNames.size ? [...releaseContextNames].sort((left, right) => left.localeCompare(right)).join(" · ") : "No release context recorded", sourceRows: explicitRecords.size, products: new Set(effectScope.results.map((row) => row.product_id).filter(Boolean)).size, releases: releaseContextNames.size, reviewRows: [...explicitRecords.values()].filter((row) => row.materialization_status !== "materialized").length, productNames, linkedRecords: linkedRecords.results.map((record) => ({ type: record.record_type, title: record.title, status: record.status, informationOrigin: record.information_origin, adjudicationAuthority: record.adjudication_authority, adjudicatedAt: record.adjudicated_at })) };
...
function briefMarkdown(title: string, initiative: InitiativeRow, snapshot: BriefSnapshot) {
  const linked = snapshot.linkedRecords.length ? snapshot.linkedRecords.map((record) => `- ${record.type}: ${record.title} (${record.status}; ${record.informationOrigin || "origin not classified"})${record.adjudicationAuthority ? `; decision authority: ${record.adjudicationAuthority}${record.adjudicatedAt ? ` on ${record.adjudicatedAt.slice(0, 10)}` : ""}` : ""}`).join("\n") : "- No linked MCPs, calls, decisions, or risks yet.";
