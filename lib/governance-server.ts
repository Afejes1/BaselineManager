...
export async function createExecutiveBrief(db: Database, actor: Actor, body: Record<string, unknown>, markdownFactory?: (context: { title: string; snapshot: BriefSnapshot }) => string, guard?: ExecutiveBriefCreationGuard) {
  ...
  const snapshot: BriefSnapshot = { asOf: now(), handlingMarking: PROGRAM_HANDLING_MARKING, releaseName: releaseContextNames.size ? [...releaseContextNames].sort((left, right) => left.localeCompare(right)).join(" · ") : "No release context recorded", sourceRows: explicitRecords.size, products: new Set(effectScope.results.map((row) => row.product_id).filter(Boolean)).size, releases: releaseContextNames.size, reviewRows: [...explicitRecords.values()].filter((row) => row.materialization_status !== "materialized").length, productNames, linkedRecords: linkedRecords.results.map((record) => ({ type: record.record_type, title: record.title, status: record.status })) };
