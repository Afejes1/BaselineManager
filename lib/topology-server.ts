...
import type { ConnectionType, InfrastructureConfidence, InfrastructureLifecycle, InfrastructureNodeType, InfrastructureOperatingState, InstallationRole, InstallationStatus, ReleaseNodeLifecycle, TopologyExtensions } from "./topology-model";
...
const infrastructureConfidences = new Set<InfrastructureConfidence>(["reported", "assessed", "confirmed"]);
...
export async function saveReleaseInfrastructureNode(db: Database, actor: Actor, body: Record<string, unknown>) {
  ...
  if (!releaseNodeLifecycles.has(lifecycleStatus) || !operatingStates.has(operatingState) || !infrastructureConfidences.has(confidence)) throw new Error("Choose valid release lifecycle, operating, and confidence states.");
  ...
  const at = now();
  await db.batch([
    db.prepare("INSERT INTO release_infrastructure_node (id,program_id,release_id,platform_id,infrastructure_node_id,parent_state_id,lifecycle_status,operating_state,confidence,cpu_cores,memory_gb,storage_gb,storage_medium_id,storage_type,drive_letter,file_system_value_id,file_system,source_reference,source_as_of,notes,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET parent_state_id=excluded.parent_state_id,lifecycle_status=excluded.lifecycle_status,operating_state=excluded.operating_state,confidence=excluded.confidence,cpu_cores=excluded.cpu_cores,memory_gb=excluded.memory_gb,storage_gb=excluded.storage_gb,storage_medium_id=excluded.storage_medium_id,storage_type=excluded.storage_type,drive_letter=excluded.drive_letter,file_system_value_id=excluded.file_system_value_id,file_system=excluded.file_system,source_reference=excluded.source_reference,source_as_of=excluded.source_as_of,notes=excluded.notes,updated_at=excluded.updated_at")
      .bind(stateId, PROGRAM_ID, releaseId, node.platform_id, infrastructureNodeId, parentStateId, lifecycleStatus, operatingState, confidence, values.cpuCores, values.memoryGb, values.storageGb, storageMedium?.id || null, storageMedium?.code || null, nullable(body.driveLetter), fileSystem?.id || null, fileSystem?.code || null, nullable(body.sourceReference), nullable(body.sourceAsOf), nullable(body.notes), actor.id, current?.created_at || at, at),
