...
export function buildInitiativeReportMarkdown(input: InitiativeReportInput) {
  ...
  let objectives = bundle.objectives.length ? bundle.objectives.map((objective) => {
    ...
    const estimates = latest.length ? latest.map((estimate) => `${readable(estimate.estimateSource)} ${number(estimate.hoursLikely)} h / ${money(estimate.costLikely)} (${date(estimate.asOf)}, ${readable(estimate.confidence)})`).join("; ") : "no sourced estimate";
  ...
  const criteria = bundle.criteria.length ? bundle.criteria.map((criterion) => {
    const signoffs = criterion.signoffs.length ? criterion.signoffs.map((signoff) => { const document = signoff.evidenceDocumentId ? documentById.get(signoff.evidenceDocumentId) : null; return `${value(signoff.signoffRole, "Unspecified role")}: ${readable(signoff.decision)}${signoff.signer ? ` by ${value(signoff.signer)}` : ""}${document ? `; evidence: [${value(document.fileName, "document")}](/api/documents?id=${encodeURIComponent(document.id)})` : signoff.evidenceDocumentId ? `; evidence document ${value(signoff.evidenceDocumentId)}` : ""}`; }).join("; ") : "no sign-off";
  ...
  let evidence = documents.length ? documents.map((document) => `- [${value(document.fileName, "document")}](/api/documents?id=${encodeURIComponent(document.id)})${document.description ? ` — ${value(document.description)}` : ""}; document ID: ${value(document.id)}; attached ${date(document.createdAt)}`).join("\n") : "- No supporting documents are attached to this Initiative.";
  const linkedRecords = baseline.linkedRecords.length ? baseline.linkedRecords.map((record) => `- ${readable(record.type)} — ${value(record.title)}; ${readable(record.status)}`).join("\n") : "- No linked calls, decisions, risks, questions, or technical notes are recorded.";
