...
export default function BriefDetailPage() {
  ...
  const [tab, setTab] = useState<"overview" | "snapshot" | "text">("overview");
  ...
  useEffect(() => { if (brief) setNotes(brief.notes || ""); }, [brief]);
  ...
  if (loading) return <DomainPageShell title="Executive brief" subtitle="Loading durable leadership output…" releaseScope="Loading"><section className="domain-section"><p className="empty">Loading brief…</p></section></DomainPageShell>;
  ...
  return <DomainPageShell title={brief.title} subtitle="Leadership-ready baseline snapshot, decision context, and publication controls." releaseScope={`${brief.snapshot.releaseName} · Snapshot ${dateLabel(brief.snapshot.asOf)}`} actions={<><button className="ghost-button" type="button" onClick={() => void exportBrief("docx")}>Download DOCX</button><button className="primary-button" type="button" onClick={() => void exportBrief("pdf")}>Download PDF</button></>}>
    <section className="kpi-grid" aria-label="Brief summary"><div className="kpi-card"><span>Lifecycle</span><strong>{displayStatus(brief.status)}</strong><small>Published {dateLabel(brief.publishedAt)}</small></div><div className="kpi-card"><span>Source scope</span><strong>{brief.snapshot.sourceRows}</strong><small>Retained source records</small></div><div className="kpi-card"><span>Products</span><strong>{brief.snapshot.products}</strong><small>Across {brief.snapshot.releases} releases</small></div><div className="kpi-card"><span>Review attention</span><strong>{brief.snapshot.reviewRows}</strong><small>Review records at snapshot time</small></div></section>
    <nav className="detail-tabs" aria-label="Brief views">{(["overview", "snapshot", "text"] as const).map((item) => <button key={item} type="button" className={tab === item ? "tab-button tab-active" : "tab-button"} onClick={() => setTab(item)}>{item === "text" ? "Brief text" : displayStatus(item)}</button>)}</nav>
    {tab === "overview" && <section className="split-layout"><article className="domain-card"><span className="eyebrow">INITIATIVE CONTEXT</span><h3>{initiative ? <Link href={`/initiatives/${encodeURIComponent(initiative.id)}`}>{initiative.title}</Link> : "Independent brief"}</h3><p>{initiative?.consequence || "No linked initiative consequence was captured."}</p><p className="entity-meta">Decision ask: {initiative?.decisionAsk || "Not recorded"}</p><p className="entity-meta">Desired outcome: {initiative?.desiredOutcome || "Not recorded"}</p></article><article className="domain-card"><span className="eyebrow">STEWARDSHIP CONTROLS</span><h3>Lifecycle and notes</h3><label className="modal-field">Status<select value={brief.status} onChange={(event) => void update({ status: event.target.value as BriefStatus }, "Brief lifecycle updated.")}>{briefStatuses.map((status) => <option key={status} value={status}>{displayStatus(status)}</option>)}</select></label><label className="modal-field">Steward notes<textarea rows={5} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Context for the next reviewer or publication audience" /></label><button className="primary-button" type="button" onClick={() => void update({ notes }, "Brief notes saved.")}>Save notes</button><p className="entity-meta">Created {dateLabel(brief.createdAt)} · Updated {dateLabel(brief.updatedAt)}</p></article></section>}
    {tab === "snapshot" && <section className="domain-section"><article className="domain-card"><div className="section-toolbar"><div><span className="eyebrow">BASELINE SNAPSHOT</span><h3>What this one-pager represents</h3></div><span>{brief.snapshot.releaseName}</span></div><p className="entity-meta">This snapshot is frozen when the brief is created; later workbook imports will not silently change a leadership artifact.</p><div className="domain-table-wrap"><table><tbody><tr><th>As of</th><td>{dateLabel(brief.snapshot.asOf)}</td></tr><tr><th>Source records</th><td>{brief.snapshot.sourceRows}</td></tr><tr><th>Products</th><td>{brief.snapshot.products}</td></tr><tr><th>Releases</th><td>{brief.snapshot.releases}</td></tr><tr><th>Review records</th><td>{brief.snapshot.reviewRows}</td></tr></tbody></table></div><h4>Representative products</h4><p>{brief.snapshot.productNames.length ? brief.snapshot.productNames.join(" · ") : "No products matched the selected scope."}</p><h4>Linked Government records</h4>{brief.snapshot.linkedRecords.length ? <ul>{brief.snapshot.linkedRecords.map((record) => <li key={`${record.type}-${record.title}`}>{displayStatus(record.type)}: {record.title} ({displayStatus(record.status)})</li>)}</ul> : <p>No MCPs, technical calls, decisions, or risks were linked at snapshot time.</p>}</article></section>}
