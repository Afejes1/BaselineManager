...
export default function BriefsPage() {
  ...
  return <DomainPageShell title="Report Archive" subtitle="Saved report snapshots and publication history." releaseScope={portfolio ? `${portfolio.actor.displayName} · ${displayStatus(portfolio.actor.role)}` : "Loading records"} actions={<><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search archived reports" /></label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as BriefStatus | "all")}><option value="all">All statuses</option>{briefStatuses.map((status) => <option key={status} value={status}>{displayStatus(status)}</option>)}</select><button className="primary-button" type="button" onClick={openCreate}>＋ Save report</button></>}>
    ...
    {!loading && !error && <section className="domain-list">{filtered.length ? filtered.map((brief) => <article className="domain-card" key={brief.id}><div className="section-toolbar"><div><span className="record-type">Saved report</span><h3><Link href={`/briefs/${encodeURIComponent(brief.id)}`}>{brief.title}</Link></h3></div><span className={`status-pill status-${brief.status}`}>{displayStatus(brief.status)}</span></div><p className="entity-meta">{brief.initiativeTitle || "Independent report"} · {brief.snapshot.releaseName}</p><p>{brief.snapshot.sourceRows} baseline records · {brief.snapshot.products} products · {brief.snapshot.reviewRows} review records at snapshot time</p><p className="entity-meta">Created {dateLabel(brief.createdAt)} · Last updated {dateLabel(brief.updatedAt)}</p><p className="entity-actions"><Link href={`/briefs/${encodeURIComponent(brief.id)}`}>Open report</Link>{brief.initiativeId && <Link href={`/initiatives/${encodeURIComponent(brief.initiativeId)}`}>Open initiative</Link>}<select aria-label={`Lifecycle for ${brief.title}`} value={brief.status} onChange={(event) => void updateStatus(brief.id, event.target.value)}>{briefStatuses.map((status) => <option key={status} value={status}>{displayStatus(status)}</option>)}</select></p></article>) : <article className="domain-card empty-state"><h3>No saved reports</h3><p>Create an Initiative, then save a report snapshot for leadership review.</p><Link href="/initiatives">Open Initiatives</Link></article>}</section>}
    {showCreate && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setShowCreate(false); }}><section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="create-brief-title"><span className="eyebrow">REPORT ARCHIVE</span><h2 id="create-brief-title">Save report snapshot</h2><p>This saves the current baseline scope, review status, products, and linked Government records.</p><label className="modal-field">Initiative<select value={effectiveInitiativeId} onChange={(event) => { const next = event.target.value; setInitiativeId(next); const item = initiatives.find((initiative) => initiative.id === next); if (item && !briefTitle) setBriefTitle(`${item.title} — Leadership Report`); }}><option value="">Select initiative</option>{initiatives.map((initiative) => <option key={initiative.id} value={initiative.id}>{initiative.title}</option>)}</select></label><label className="modal-field">Report title<input value={briefTitle} onChange={(event) => setBriefTitle(event.target.value)} placeholder="Report title" /></label>{selectedInitiative && <div className="preview-card"><strong>{selectedInitiative.primaryReleaseName || "All releases"}</strong><span>{selectedInitiative.scope.filter((scope) => scope.scopeKind === "product").length || "All"} products in initiative scope · {selectedInitiative.linkedRecordCount} linked Government records</span></div>}<footer><button className="ghost-button" type="button" disabled={saving} onClick={() => setShowCreate(false)}>Cancel</button><button className="primary-button" type="button" disabled={saving} onClick={() => void createBrief()}>{saving ? "Saving…" : "Save report"}</button></footer></section></div>}
