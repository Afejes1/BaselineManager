...
export function BaselineManager() {
  ...
  const [activeQuality, setActiveQuality] = useState("All quality");
  ...
  const filtered = useMemo(() => rows.map((row,index) => ({ row,index })).filter(({row}) => {
    ...
    const qualityMatch = activeQuality === "All quality" || dataQualityFor(row).label === activeQuality;
    return releaseMatch && tierMatch && qualityMatch && TECHNICAL_BASELINE_COLUMNS.map((column) => text(row[column])).join(" ").toLowerCase().includes(query.toLowerCase());
  }), [rows, query, activeRelease, activeTier, activeQuality]);
  ...
  function addRow() {
    ...
    const next = [...rows,row]; setSelectedIndex(rows.length); persist(next, `Created a new source occurrence in release ${chosenRelease}.`); setActiveTier("All records"); setActiveQuality("All quality"); setShowAll(true); setShowAddRow(false);
  }
  ...
  async function acceptImport() {
    if (!draft) return;
    persist(draft.rows, `Imported ${draft.rows.length} rows across ${new Set(draft.rows.map(releaseOf)).size} releases from ${draft.fileName}.`); setSelectedIndex(0); setActiveRelease("All releases"); setActiveTier("All records"); setActiveQuality("All quality");
  ...
  return <main className="shell">
    ...
    <aside className="rail"><div className="brand"><span className="brand-mark">V3</span><span>JSF Baseline</span></div><nav aria-label="Primary navigation"><p className="rail-label">Workspace</p>{nav.map((item,index)=><button key={item} className={index===0?"nav-item active":"nav-item"}><span>{["▦","⇩","◇","⌁","▤"][index]}</span>{item}{index>2&&<em>Soon</em>}</button>)}</nav><div className="rail-context"><span className="context-dot"/><div><strong>Release scope</strong><small>{activeRelease} · Reported</small></div></div><button className="profile"><span>AC</span><div><strong>Baseline steward</strong><small>Government team</small></div><b>···</b></button></aside>
    <section className="workspace"><header className="topbar"><div><span className="eyebrow">TECHNICAL BASELINE</span><h1>Baseline Manager</h1></div><div className="top-actions"><label className="release-selector"><span>Release scope</span><select value={activeRelease} onChange={(event)=>{setActiveRelease(event.target.value);setActiveTier("All records");}}><option>All releases</option>{releases.map((release)=><option key={release}>{release}</option>)}</select></label><button className="ghost-button">Activity</button><button className="primary-button" onClick={()=>fileRef.current?.click()}>Import workbook</button></div></header>
      <section className="summary"><div className="summary-lead"><p>{activeRelease === "All releases" ? `${releases.length} RELEASES IN SCOPE` : `RELEASE ${activeRelease}`}</p><h2>{activeRelease === "All releases" ? "Reported baselines across releases" : "Reported technical baseline"}</h2><span>Working draft · ReleaseName retained on every source occurrence</span></div><div className="metric"><span>Source records</span><strong>{scopeRows.length}</strong><small>{activeRelease} · exact projection</small></div><div className="metric"><span>Canonical products</span><strong>{productCount}</strong><small>Across {scopeTiers.size} tiers in scope</small></div><div className="metric metric-alert"><span>Needs attention</span><strong>{issueCount}</strong><small>{scopeRows.filter(r=>dataQualityFor(r).level==="issue").length} blocking · {scopeRows.filter(r=>dataQualityFor(r).level==="review").length} review</small></div></section>
      <div className="content-grid"><aside className="tree-panel"><div className="panel-heading"><div><span className="eyebrow">STRUCTURE</span><h3>Release configuration</h3></div><button>•••</button></div><div className="tree-list"><button className={activeRelease==="All releases"&&activeTier==="All records"?"tree-row selected":"tree-row"} onClick={()=>{setActiveRelease("All releases");setActiveTier("All records");}}><span>▦</span><b>All releases</b><em>{rows.length}</em></button>{releaseGroups.map((group)=><div className="release-tree" key={group.release}><button className={activeRelease===group.release&&activeTier==="All records"?"tree-row release-row selected":"tree-row release-row"} onClick={()=>{setActiveRelease(group.release);setActiveTier("All records");}}><span>◆</span><b>{group.release}</b><em>{group.rows.length}</em></button>{group.tiers.map((tier)=><button key={`${group.release}:${tier}`} className={activeRelease===group.release&&activeTier===tier?"tree-row tree-child selected":"tree-row tree-child"} onClick={()=>{setActiveRelease(group.release);setActiveTier(tier);}}><span>└</span><b>{tier}</b><em>{group.rows.filter((row)=>tierOf(row)===tier).length}</em></button>)}</div>)}</div><div className="quality-card"><span className="quality-score">{scopeRows.length?Math.round((scopeRows.length-issueCount)/scopeRows.length*100):100}%</span><div><strong>Data quality</strong><small>{scopeRows.length-issueCount} of {scopeRows.length} complete in scope</small></div></div></aside>
        <section className="records-panel"><div className="records-toolbar"><label className="search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search products, hosts, or OEM…"/></label><button className={showFilters?"tool-button tool-active":"tool-button"} onClick={()=>setShowFilters((value)=>!value)} aria-expanded={showFilters}>≡ Filter <span>{(activeRelease==="All releases"?0:1)+(activeTier==="All records"?0:1)+(activeQuality==="All quality"?0:1)}</span></button><button className="tool-button" onClick={()=>setShowAll(true)}>24 columns</button><div className="spacer"/><button className="tool-button" onClick={exportWorkbook}>Export {activeRelease==="All releases"?"all":activeRelease} .xlsx</button><button className="add-button" onClick={openAddRow}>＋ Add row</button></div>
          {showFilters&&<section className="filter-panel" aria-label="Source record filters"><div><span>ReleaseName</span><select value={activeRelease} onChange={(event)=>{setActiveRelease(event.target.value);setActiveTier("All records");}}><option>All releases</option>{releases.map((release)=><option key={release}>{release}</option>)}</select></div><div><span>Tier</span><select value={activeTier} onChange={(event)=>setActiveTier(event.target.value)}><option>All records</option>{availableTiers.map((tier)=><option key={tier}>{tier}</option>)}</select></div><div><span>Data quality</span><select value={activeQuality} onChange={(event)=>setActiveQuality(event.target.value)}><option>All quality</option><option>Complete</option><option>Review</option><option>Blocking</option></select></div><button onClick={()=>{setActiveRelease("All releases");setActiveTier("All records");setActiveQuality("All quality");setQuery("");}}>Clear filters</button></section>}
          <div className="table-wrap"><table><thead><tr><th><input type="checkbox" aria-label="Select all visible rows" checked={filtered.length>0&&filtered.every(({index})=>checked.has(index))} onChange={()=>setChecked(filtered.every(({index})=>checked.has(index))?new Set():new Set(filtered.map(({index})=>index)))}/></th><th>#</th><th>Release</th><th>Product</th><th>Placement</th><th>Host</th><th>Type</th><th>OEM</th><th>Runtime</th><th><span className="quality-heading">Data quality<button type="button" className="quality-info" aria-label="Explain calculated data quality" onClick={()=>setShowQualityHelp(true)}>?</button></span></th><th></th></tr></thead><tbody>{filtered.map(({row,index})=>{const key=text(row["#"]);const quality=dataQualityFor(row);return <tr key={`${text(row.ReleaseName)}:${key}:${index}`} className={selectedIndex===index?"row-selected":""} onClick={()=>setSelectedIndex(index)}><td><input type="checkbox" checked={checked.has(index)} onClick={e=>e.stopPropagation()} onChange={()=>toggleChecked(index)} aria-label={`Select ${text(row.LongName)||key} in ${text(row.ReleaseName)}`}/></td><td className="mono">{key}</td><td><span className="release-chip">{text(row.ReleaseName)||"Unassigned"}</span></td><td><strong>{text(row.ShortName)||"Unnamed"}</strong><small>{text(row.LongName)||"Canonical name missing"}</small></td><td><strong>{text(row.Tier)||"Unassigned"}</strong><small>{text(row.Resource)||"Resource missing"}</small></td><td className="mono">{text(row.HW_Host)||"—"}</td><td>{text(row.TechStackType)||"—"}</td><td>{text(row.OEM)||"—"}</td><td>{text(row.Containerized)==="Yes"?`${text(row.Containerized)} · ${text(row["Container Technology"])}`:text(row.Containerized)||"—"}</td><td><Mark quality={quality}/></td><td>›</td></tr>})}</tbody></table>{!filtered.length&&<div className="empty">No source records match this release, tier, and data-quality scope.</div>}</div><footer className="table-footer"><span>Showing {filtered.length} records · {scopeRows.length} in {activeRelease}</span><div><button disabled>‹</button><b>1</b><button>›</button></div></footer></section>
        <aside className="detail-panel"><div className="detail-head"><div><span className="eyebrow">SOURCE RECORD #{text(selected["#"])||"NEW"}</span><h3>{text(selected.ShortName)||"New product"}</h3><p>{text(selected.LongName)||"Complete the retained source columns."}</p></div><button>×</button></div><div className="detail-tabs"><button className="tab-active">Record</button><button>Lineage</button><button>History</button></div><div className="detail-body"><div className="detail-status quality-summary"><Mark quality={selectedQuality}/><div><strong>Calculated data quality</strong><span>Not a source column · not included in XLSX export</span></div><button type="button" className="quality-info" aria-label="Explain calculated data quality" onClick={()=>setShowQualityHelp(true)}>?</button></div>
          <section className="quality-checks"><h4>Data quality checks</h4>{selectedQuality.issues.length===0?<p className="quality-complete">✓ No validation issues found in the current source values.</p>:<ul>{selectedQuality.issues.map((issue,index)=><li key={`${issue.field}:${index}`} className={`quality-${issue.severity}`}><strong>{issue.field}</strong><span>{issue.message}</span></li>)}</ul>}<p className="quality-guidance">Edit the flagged source fields below. Data quality updates automatically; it is not manually assigned.</p></section>
    ...
    {showQualityHelp&&<div className="modal-backdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setShowQualityHelp(false);}}><section className="import-modal quality-help-modal" role="dialog" aria-modal="true" aria-labelledby="quality-help-title"><span className="eyebrow">CALCULATED DATA QUALITY</span><h2 id="quality-help-title">What does Data quality mean?</h2><p>Data quality is a validation result calculated from the retained source values. It is <strong>not one of the 24 spreadsheet columns</strong>, cannot be manually assigned, and is not included in the XLSX export.</p><div className="quality-key"><div><Mark quality={{level:"ready",label:"Complete",issues:[]}}/><span>No validation issues were found.</span></div><div><Mark quality={{level:"review",label:"Review",issues:[]}}/><span>The row is usable, but a value is incomplete or inconsistent.</span></div><div><Mark quality={{level:"issue",label:"Blocking",issues:[]}}/><span>The row cannot be reliably materialized until a required identity is corrected.</span></div></div><p className="modal-note">Select a row to see its exact validation reasons. Correct the listed source fields and the result updates automatically.</p><footer><button className="primary-button" onClick={()=>setShowQualityHelp(false)}>Got it</button></footer></section></div>}
    {(draft||importError)&&<div className="modal-backdrop" role="presentation"><section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="import-title"><span className="eyebrow">WORKBOOK INTAKE</span><h2 id="import-title">{importError?"Contract mismatch":"Ready to reconcile"}</h2>{importError?<><p className="error-copy">{importError}</p><div className="contract-strip">Expected: {TECHNICAL_BASELINE_COLUMNS.length} columns · exact names · exact order</div><footer><button className="primary-button" onClick={()=>setImportError("")}>Return to baseline</button></footer></>:draft&&<><p><strong>{draft.fileName}</strong> · {draft.sheetName}</p><div className="import-stats four"><div><strong>{draft.rows.length}</strong><span>Source rows</span></div><div><strong>{new Set(draft.rows.map(releaseOf)).size}</strong><span>Releases</span></div><div><strong>{draft.rows.filter(r=>dataQualityFor(r).level==="ready").length}</strong><span>Complete</span></div><div><strong>{draft.rows.filter(r=>dataQualityFor(r).level!=="ready").length}</strong><span>Needs attention</span></div></div><div className="release-list"><span>ReleaseName values</span>{Array.from(new Set(draft.rows.map(releaseOf))).map((release)=><b key={release}>{release} · {draft.rows.filter((row)=>releaseOf(row)===release).length} rows</b>)}</div><p className="modal-note">Each source occurrence retains ReleaseName. Import reuses the canonical product while linking its reported configuration and deployment state to the correct release baseline.</p><footer><button className="ghost-button" onClick={()=>setDraft(null)}>Cancel</button><button className="primary-button" onClick={acceptImport}>Import and reconcile</button></footer></>}</section></div>}
