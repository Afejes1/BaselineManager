...
export function BaselineManager() {
  ...
  return <main className="shell">
    ...
    <section className="workspace"><header className="topbar"><div><span className="eyebrow">TECHNICAL BASELINE</span><h1>Baseline Manager</h1></div><div className="top-actions"><label className="release-selector"><span>Release scope</span><select value={activeRelease} onChange={(event)=>{setActiveRelease(event.target.value);setActiveTier("All records");}}><option>All releases</option>{releases.map((release)=><option key={release}>{release}</option>)}</select></label><button className="ghost-button">Activity</button><button className="primary-button" onClick={()=>fileRef.current?.click()}>Import workbook</button></div></header>
      ...
      {selectedIndex!==null&&<div className="detail-backdrop record-overlay" role="presentation"><aside className="detail-panel detail-sheet record-page" role="dialog" aria-modal="true" aria-label={`Source record ${text(selected["#"])}`}><div className="detail-head"><button className="ghost-button record-back-button" type="button" onClick={()=>{setSelectedIndex(null);setShowAll(false);}}>← Back to grid</button><div><span className="eyebrow">SOURCE RECORD #{text(selected["#"])||"NEW"}</span><h3>{text(selected.ShortName)||"New product"}</h3><p>{text(selected.LongName)||"Complete the retained source columns."}</p><span className="autosave-label">✓ Changes save automatically</span></div><button type="button" aria-label="Close record details" title="Close" onClick={()=>{setSelectedIndex(null);setShowAll(false);}}>×</button></div><div className="detail-tabs"><button className="tab-active">Record</button><button>Lineage</button><button>History</button></div><div className="detail-body"><div className="detail-status quality-summary"><Mark quality={selectedQuality}/><div><strong>Automated health checks</strong><span>Calculated from source values · not included in XLSX export</span></div><button type="button" className="quality-info" aria-label="Explain automated health checks" onClick={()=>setShowQualityHelp(true)}>?</button></div>
