...
export function BaselineManager() {
  ...
  const filtered = useMemo(() => rows.map((row,index) => ({ row,index })).filter(({row}) => {
    const releaseMatch = activeRelease === "All releases" || text(row.ReleaseName) === activeRelease;
    const tierMatch = activeTier === "All records" || text(row.Tier) === activeTier;
    return releaseMatch && tierMatch && TECHNICAL_BASELINE_COLUMNS.map((column) => text(row[column])).join(" ").toLowerCase().includes(query.toLowerCase());
  }), [rows, query, activeRelease, activeTier]);
  const releases = Array.from(new Set(rows.map((row) => text(row.ReleaseName) || "Unassigned")));
  const releaseGroups = releases.map((release) => {
    const releaseRows = rows.filter((row) => (text(row.ReleaseName) || "Unassigned") === release);
    return { release, rows:releaseRows, tiers:Array.from(new Set(releaseRows.map((row) => text(row.Tier) || "Unassigned"))) };
  });
  const scopeRows = activeRelease === "All releases" ? rows : rows.filter((row) => (text(row.ReleaseName) || "Unassigned") === activeRelease);
  const scopeTiers = new Set(scopeRows.map((row) => text(row.Tier)).filter(Boolean));
  const issueCount = scopeRows.filter((row) => issueFor(row) !== "ready").length;
  ...
  function addRow() {
    ...
    const row = { ...blankRecord(), "#": String(nextNumber).padStart(6, "0"), ReleaseName: activeRelease === "All releases" ? text(rows[0]?.ReleaseName) : activeRelease };
    setSelectedIndex(rows.length); setRows((current) => [...current, row]); setShowAll(true);
  }
  ...
  function bulkReview() {
    ...
    const next = rows.map((row,index) => checked.has(index) ? { ...row, Notes: text(row.Notes) || "Flagged for baseline steward review." } : row);
  ...
  async function acceptImport() {
    if (!draft) return;
    persist(draft.rows, `Imported ${draft.rows.length} rows across ${new Set(draft.rows.map((row) => text(row.ReleaseName) || "Unassigned")).size} releases from ${draft.fileName}.`); setSelectedIndex(0); setActiveRelease("All releases"); setActiveTier("All records");
  ...
  return <main className="shell">
    ...
    {(draft||importError)&&<div className="modal-backdrop" role="presentation"><section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="import-title"><span className="eyebrow">WORKBOOK INTAKE</span><h2 id="import-title">{importError?"Contract mismatch":"Ready to reconcile"}</h2>{importError?<><p className="error-copy">{importError}</p><div className="contract-strip">Expected: {TECHNICAL_BASELINE_COLUMNS.length} columns · exact names · exact order</div><footer><button className="primary-button" onClick={()=>setImportError("")}>Return to baseline</button></footer></>:draft&&<><p><strong>{draft.fileName}</strong> · {draft.sheetName}</p><div className="import-stats four"><div><strong>{draft.rows.length}</strong><span>Source rows</span></div><div><strong>{new Set(draft.rows.map((row)=>text(row.ReleaseName)||"Unassigned")).size}</strong><span>Releases</span></div><div><strong>{draft.rows.filter(r=>issueFor(r)==="ready").length}</strong><span>Ready</span></div><div><strong>{draft.rows.filter(r=>issueFor(r)!=="ready").length}</strong><span>Review</span></div></div><div className="release-list"><span>ReleaseName values</span>{Array.from(new Set(draft.rows.map((row)=>text(row.ReleaseName)||"Unassigned"))).map((release)=><b key={release}>{release} · {draft.rows.filter((row)=>(text(row.ReleaseName)||"Unassigned")===release).length} rows</b>)}</div><p className="modal-note">Each source occurrence retains ReleaseName. Import reuses the canonical product while linking its reported configuration and deployment state to the correct release baseline.</p><footer><button className="ghost-button" onClick={()=>setDraft(null)}>Cancel</button><button className="primary-button" onClick={acceptImport}>Import and reconcile</button></footer></>}</section></div>}
