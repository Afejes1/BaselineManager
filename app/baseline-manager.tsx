...
export function BaselineManager() {
  ...
  const filtered = useMemo(() => rows.map((row,index) => ({ row,index })).filter(({row}) => {
    ...
    const statusMatch = activeStatus === "All statuses" || issueFor(row) === activeStatus.toLowerCase();
    return releaseMatch && tierMatch && statusMatch && TECHNICAL_BASELINE_COLUMNS.map((column) => text(row[column])).join(" ").toLowerCase().includes(query.toLowerCase());
  }), [rows, query, activeRelease, activeTier, activeStatus]);
  ...
  function addRow() {
    ...
    const next = [...rows,row]; setSelectedIndex(rows.length); persist(next, `Created a new source occurrence in release ${chosenRelease}.`); setActiveTier("All records"); setActiveStatus("All statuses"); setShowAll(true); setShowAddRow(false);
  }
  ...
  async function acceptImport() {
    if (!draft) return;
    persist(draft.rows, `Imported ${draft.rows.length} rows across ${new Set(draft.rows.map(releaseOf)).size} releases from ${draft.fileName}.`); setSelectedIndex(0); setActiveRelease("All releases"); setActiveTier("All records"); setActiveStatus("All statuses");
