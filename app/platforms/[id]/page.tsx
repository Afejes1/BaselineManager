...
export default function PlatformDetailPage() {
  ...
  return <DomainPageShell title={sourcePlatform ? `Resource Platform: ${platform.name}` : `${platform.code} · ${platform.name}`} subtitle={sourcePlatform ? `A2O Resource Platform · Tier descriptor: ${platform.reportedTierName || "not reported"}` : `${platform.platformType.toUpperCase()} Platform dashboard`} releaseScope={selectedReleaseText} contextMode="filter" objectContext={{ kind: "platform", id, label: sourcePlatform ? `Resource Platform · ${platform.name}` : `${platform.code} · ${platform.name}` }} actions={<><Link className="ghost-button" href={`/platforms${releaseQuery}`}>Platforms</Link><AnalyticsLink kind="platform" id={id} /><button className="ghost-button" type="button" onClick={openEdit}>Edit Platform context</button><button className="ghost-button" type="button" onClick={() => setTab("baseline")}>Edit Release mapping</button><button className="ghost-button" type="button" onClick={() => setTab("infrastructure")}>Edit system configuration</button><button className="ghost-button" type="button" onClick={() => window.print()}>Print dashboard</button></>}>
    ...
    <section className="contract-strip"><strong>Editing model</strong><span><em>Platform context</em> is the durable Government hierarchy and can be edited here. <em>Baseline assignments</em> are release-specific mappings. <em>System configuration</em> is release-specific infrastructure and installation detail. Imported A2O Resource and Tier values remain traceable source context; a Government mapping is not overwritten by a later import.</span></section>
