...
export default function PlatformDetailPage() {
  ...
  return <DomainPageShell title={`${platform.code} · ${platform.name}`} subtitle={`${platform.platformType.toUpperCase()} Platform dashboard`} releaseScope={`${releases.length} releases`} actions={<><Link className="ghost-button" href="/platforms">Hierarchy</Link><button className="ghost-button" type="button" onClick={() => window.print()}>Print dashboard</button></>}>
    ...
    <section className="dashboard-grid"><article className="domain-card"><span className="eyebrow">HIERARCHY CONTEXT</span><h3>{parent ? <Link href={`/platforms/${encodeURIComponent(parent.id)}`}>{parent.code} · {parent.name}</Link> : "Program root"} → {platform.code}</h3><p>{platform.description || "No governed description has been recorded."}</p><p className="entity-meta">{platform.installationLocation || "Location not recorded"}{platform.countryCode ? ` · ${platform.countryCode}` : ""} · {platform.status}</p></article><article className="domain-card"><span className="eyebrow">WHAT / WHERE / WHEN</span><h3>{products.length} products at {platform.code}</h3><p>{releases.length ? `Reported across ${releases.join(", ")}.` : "Link this Platform to a configuration node to roll up source occurrences."}</p><p className="entity-actions"><Link href="/reports">Open deterministic reports</Link></p></article></section>
