...
export default function PlatformsPage() {
  ...
  return <DomainPageShell title="Platform hierarchy" subtitle="Installation context from global node to fielded endpoint" releaseScope={`${portfolio.platforms.length} governed platforms`} actions={<button className="primary-button" type="button" onClick={() => setOpen(true)}>+ New platform</button>}>
    <section className="summary"><div className="metric"><span>ALOU global nodes</span><strong>{metrics.alou}</strong><small>Program-wide roots</small></div><div className="metric"><span>OCK country nodes</span><strong>{metrics.ock}</strong><small>Country context</small></div><div className="metric"><span>OBK sites</span><strong>{metrics.obk}</strong><small>Squadron / installation</small></div><div className="metric"><span>PMA endpoints</span><strong>{metrics.pma}</strong><small>Laptops and endpoints</small></div></section>
    <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">GOVERNED INSTALLATION TREE</span><h3>Where the baseline is installed</h3></div><span>Open a node for products, releases, organizations, and Change Request effects</span></div>
    ...
    {open ? <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setOpen(false); }}><section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="platform-title"><span className="eyebrow">GOVERNED EXTENSION</span><h2 id="platform-title">Create Platform</h2><p>Platform describes the installation hierarchy. It does not alter the 24-column workbook contract.</p><div className="form-grid"><label className="modal-field">Type<select value={form.platformType} onChange={(event) => setForm({ ...form, platformType: event.target.value as PlatformType })}>{Object.entries(typeLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="modal-field">Code<input value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} placeholder="e.g., OBK-VA-07" /></label><label className="modal-field">Name<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Installation or node name" /></label><label className="modal-field">Parent<select value={form.parentId} onChange={(event) => setForm({ ...form, parentId: event.target.value })}><option value="">No parent / root</option>{portfolio.platforms.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label><label className="modal-field">Location<input value={form.installationLocation} onChange={(event) => setForm({ ...form, installationLocation: event.target.value })} /></label><label className="modal-field">Country code<input value={form.countryCode} maxLength={3} onChange={(event) => setForm({ ...form, countryCode: event.target.value })} /></label></div><label className="modal-field">Description<textarea rows={3} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>{message ? <p className="error-copy">{message}</p> : null}<footer><button className="ghost-button" type="button" onClick={() => setOpen(false)}>Cancel</button><button className="primary-button" type="button" disabled={saving} onClick={() => void save()}>{saving ? "Saving…" : "Create platform"}</button></footer></section></div> : null}
  </DomainPageShell>;
}
