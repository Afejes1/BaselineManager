...
function PlatformBranch({ platform, all, depth = 0 }: { platform: PlatformRecord; all: PlatformRecord[]; depth?: number }) {
  ...
  return <div className="platform-branch" style={{ "--platform-depth": depth } as React.CSSProperties}>
    <Link className={`platform-node platform-${platform.platformType}`} href={`/platforms/${encodeURIComponent(platform.id)}`}>
      <span className="platform-type">{sourcePlatform ? "A2O RESOURCE" : platform.platformType.toUpperCase()}</span>
      <strong>{sourcePlatform ? platform.name : `${platform.code} · ${platform.name}`}</strong>
      <small>{sourcePlatform ? `Tier descriptor: ${platform.reportedTierName || "not reported"} · ` : ""}{platform.directOccurrenceCount} direct baseline records · {platform.directProductCount} products · {platform.status}</small>
    </Link>
...
export default function PlatformsPage() {
  ...
  return <DomainPageShell title="Platforms" subtitle="A2O Resource is the reported Platform; Tier is its descriptor. Government fielding hierarchy remains separately governed." releaseScope={`${portfolio.platforms.length} platform records`} actions={<button className="primary-button" type="button" onClick={() => setOpen(true)}>+ New platform</button>}>
    <section className="summary"><div className="metric"><span>A2O Resource Platforms</span><strong>{metrics.a2o}</strong><small>Materialized from Resource</small></div><div className="metric"><span>ALOU / OCK</span><strong>{metrics.alou} / {metrics.ock}</strong><small>Global and country nodes</small></div><div className="metric"><span>OBK / PMA</span><strong>{metrics.obk} / {metrics.pma}</strong><small>Site and endpoint nodes</small></div><div className={`metric ${unmapped ? "metric-alert" : ""}`}><span>Unmapped baseline records</span><strong>{unmapped}</strong><small>Require a primary Platform assignment</small></div></section>
    <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">A2O SOURCE PLATFORMS</span><h3>Resource Platforms reported in the A2O Tech Stack</h3></div><span>Resource is the Platform · Tier is its descriptor</span></div>
      {loading ? <p>Loading Platforms…</p> : error ? <p className="error-copy">{error}</p> : sourcePlatforms.length ? <div className="platform-tree">{sourcePlatforms.map((platform) => <PlatformBranch key={platform.id} platform={platform} all={portfolio.platforms} />)}</div> : <article className="domain-card empty-state"><h3>No A2O Resource Platforms yet</h3><p>Import an A2O Tech Stack workbook. Every reported Resource will create or update a Platform with its Tier descriptor.</p></article>}
    </section>
    <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">GOVERNED FIELDING HIERARCHY</span><h3>ALOU, OCK, OBK, and PMA context</h3></div><span>Open a node for products, releases, organizations, and Change Request effects</span></div>
