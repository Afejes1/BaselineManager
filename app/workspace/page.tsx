  ...
  return <DomainPageShell title="Workspace Transfer" subtitle="Move the complete governed dataset between application deployments." contextMode="portfolio" breadcrumb={[{ label: "Workspace Transfer" }]}>
    ...
    <section className="domain-list workspace-transfer-grid">
      <article className="domain-card transfer-card">
        ...
        <p>Exports baseline records, master data, Releases, Platforms, Change Request references, LM Objectives, Initiative decision cases, option plans, requirements, acceptance, call records, archived audit history, and attached evidence. Restore preserves the destination audit trail and records the import as a new event.</p>
    ...
    <section className="domain-card transfer-rules"><span className="eyebrow">TRANSFER RULES</span><h2>Controlled movement</h2><ol><li>Export the current workspace before an application upgrade or machine move.</li><li>Retain the package with the application version and transfer date.</li><li>Validate the package in the destination.</li><li>Replace only an empty workspace or a workspace whose current package has been secured.</li><li>Run Analyst Control and review Initiative decision-basis integrity after import.</li></ol></section>
