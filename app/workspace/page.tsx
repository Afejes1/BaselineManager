  ...
  return <DomainPageShell title="Workspace Transfer" subtitle="Move the complete governed dataset between application deployments." contextMode="portfolio" breadcrumb={[{ label: "Workspace Transfer" }]}>
    <section className="domain-list workspace-transfer-grid">
      <article className="domain-card transfer-card">
        ...
        <p>Exports baseline records, master data, Releases, Platforms, Change Request references, LM Objectives, Initiatives, WBS, requirements, acceptance, call records, audit history, and attached evidence.</p>
        ...
        <button className="primary-button" type="button" disabled={busy !== null} onClick={() => void download()}>{busy === "export" ? "Creating package…" : "Export full workspace"}</button>
      </article>
      <article className="domain-card transfer-card">
        ...
        <label className="modal-field">Workspace package<input type="file" accept=".a2oworkspace,application/zip" onChange={(event) => { setFile(event.target.files?.[0] || null); setPreview(null); setConfirmation(""); setError(""); }} /></label>
        <button className="ghost-button" type="button" disabled={!file || busy !== null} onClick={() => void submit("validate")}>{busy === "validate" ? "Validating…" : "Validate package"}</button>
