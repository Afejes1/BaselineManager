...
export async function POST(request: Request) {
  try {
    ...
    // A node/deployment has one shared state per release working baseline. When
    // the denormalized exchange file contains incompatible claims, retain every intake
    ...
    add("package", "INSERT INTO source_package (id,program_id,source_system,file_name,sheet_name,content_hash,received_at,status,row_count,accepted_count,exception_count,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING", packageId,programId,"a2o-xlsx",body.fileName,body.sheetName??null,stableId("hash",packageFingerprint),now,acceptedCount === body.rows.length ? "materialized" : "materialized_with_exceptions",body.rows.length,acceptedCount,body.rows.length-acceptedCount,now,now);
    add("workspace", "INSERT INTO baseline_workspace (id,program_id,label,active_import_package_id,created_at,updated_at) VALUES (?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET label=excluded.label,active_import_package_id=excluded.active_import_package_id,updated_at=excluded.updated_at", workspaceId,programId,"Working Technical Baseline",packageId,now,now);
    ...
    body.rows.forEach((row,rowIndex) => {
      ...
      add(`release:${releaseId}`, "INSERT INTO release (id,program_id,code,normalized_code,name,normalized_name,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,status=excluded.status,updated_at=excluded.updated_at", releaseId,programId,releaseName,normalized(releaseName),releaseName,normalized(releaseName),"working",now,now);
      ...
      statements.push(db.prepare("INSERT INTO source_row_24 (id,source_package_id,source_key,row_number,row_hash,raw_payload,release_name,tier,resource,tech_stack_type,short_name,hw_host,hw_storage_type,hw_storage_gb,hw_cpu_cores,hw_ram_gb,sw_language,software_type,oem,containerized,container_technology,container_type,long_name,notes,capability_notes,notes_1,notes_2,notes_3,notes_4,release_id,baseline_id,configuration_node_id,product_id,deployment_id,materialization_status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING").bind(sourceRowId,packageId,cell(row["#"]),rowIndex+2,stableId("hash",JSON.stringify(row)),JSON.stringify(row),cell(row.ReleaseName),cell(row.Tier),cell(row.Resource),cell(row.TechStackType),cell(row.ShortName),cell(row.HW_Host),cell(row.HW_Storage_Type),cell(row["HW_Storage (GB)"]),cell(row.HW_CPU_CORES),cell(row["HW_RAM (GB)"]),cell(row["SW Language"]),cell(row["Software Type"]),oem,cell(row.Containerized),cell(row["Container Technology"]),cell(row["Container Type"]),cell(row.LongName),cell(row.Notes),cell(row["Technical Capability Satisfied by this SW/Tech - Notes"]),cell(row["Notes.1"]),cell(row["Notes.2"]),cell(row["Notes.3"]),cell(row["Notes.4"]),releaseId,workingBaselineId,hostId,productId,deploymentId,materializationStatus,now,now));
