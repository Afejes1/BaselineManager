...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { fileName?: string; sheetName?: string; rows?: IncomingRow[]; replaceActiveBaseline?: boolean };
    ...
    const now = nowIso(); const hash = await sha256(JSON.stringify(incoming));
    ...
    for (let index = 0; index < incoming.length; index += 1) {
      const row = incoming[index]; const existingRecord = existingByIdentity.get(intakeIdentity(row)); const sourceRowId = existingRecord?.sourceRowId || crypto.randomUUID();
    ...
    statements.push(audit(env.DB, actor, replaceActiveBaseline ? "demonstration_baseline_loaded" : "a2o_intake_reconciled", "source_package", packageId, { rows: incoming.length, added, updated, unchanged: reconciliation.unchanged.length, archivedActiveRecords: replaceActiveBaseline ? existing.map((record) => record.occurrenceId) : [], absent: absent.map((record) => record.occurrenceId) }));
