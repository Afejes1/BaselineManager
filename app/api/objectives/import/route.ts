...
export async function POST(request: Request) {
  try {
    ...
    for (const item of preview.rows) {
      ...
      if (item.disposition === "add") statements.push(env.DB.prepare("INSERT INTO incumbent_objective (id,program_id,change_request_id,external_system,external_identifier,external_item_type,title,summary,technical_owner,status,planned_start,planned_finish,actual_start,actual_finish,source_locator,source_as_of,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(objectiveId, PROGRAM_ID, item.owningChangeRequestId, source.ExternalSystem, source.ExternalIdentifier, source.ExternalItemType, source.Title, source.Summary || null, source.TechnicalOwner || null, source.Status, source.PlannedStart || null, source.PlannedFinish || null, source.ActualStart || null, source.ActualFinish || null, source.SourceLocator || null, source.SourceAsOf, actor.id, at, at));
      if (item.disposition === "change") statements.push(env.DB.prepare("UPDATE incumbent_objective SET external_item_type=?,title=?,summary=?,technical_owner=?,status=?,planned_start=?,planned_finish=?,actual_start=?,actual_finish=?,source_locator=?,source_as_of=?,updated_at=? WHERE id=? AND change_request_id=?").bind(source.ExternalItemType, source.Title, source.Summary || null, source.TechnicalOwner || null, source.Status, source.PlannedStart || null, source.PlannedFinish || null, source.ActualStart || null, source.ActualFinish || null, source.SourceLocator || null, source.SourceAsOf, at, objectiveId, item.owningChangeRequestId));
