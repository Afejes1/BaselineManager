...
export default function SourceOccurrencePage() {
  ...
  return (
    ...
    >
      ...
      {tab === "source" ? <section className="domain-section"><div className="section-heading"><h3>24-column record</h3><span>Revision {row.__meta.revision} · imported values retained</span></div><div className="domain-table-wrap"><table><tbody>{TECHNICAL_BASELINE_COLUMNS.map((column) => <tr key={column}><th>{column}</th><td>{text(row[column]) || "—"}</td></tr>)}</tbody></table></div></section> : null}
