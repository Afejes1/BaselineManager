  ...
  return (
    ...
    >
      <section className="detail-tabs" aria-label="Baseline record detail tabs">
        <button type="button" role="tab" aria-selected={tab === "source"} className={`tab-button ${tab === "source" ? "tab-active" : ""}`} onClick={() => setTab("source")}>A2O Tech Stack</button>
      ...
      {tab === "source" ? <section className="domain-section"><div className="section-heading"><h3>A2O Tech Stack record</h3><span>Revision {row.__meta.revision} · imported values retained</span></div><div className="domain-table-wrap"><table><tbody>{TECHNICAL_BASELINE_COLUMNS.map((column) => <tr key={column}><th>{column}</th><td>{text(row[column]) || "—"}</td></tr>)}</tbody></table></div></section> : null}
