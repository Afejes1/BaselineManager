...
export default function BaselineRecordReferencePage() {
  ...
  return (
    ...
    >
      ...
      {tab === "normalized" ? <><section className="normal-grid"><div className="normal-card"><h5>Working baseline</h5><p><strong>Release</strong>{text(row.ReleaseName) || "Unassigned"}</p><p><strong>Baseline</strong>{row.__meta.baseline.name || "Working Technical Baseline"}</p><p><strong>Maturity</strong>{row.__meta.baseline.maturity || "working"}</p></div><div className="normal-card"><h5>Reported placement</h5><p><strong>Tier</strong>{text(row.Tier) || "Unassigned"}</p><p><strong>Resource</strong>{text(row.Resource) || "Unassigned"}</p><p><strong>Host</strong>{text(row.HW_Host) || "Unassigned"}</p></div><div className="normal-card"><h5>Product / runtime</h5><p><strong>Product</strong>{product}</p><p><strong>Supplier</strong>{text(row.OEM) || "Unassigned"}</p><p><strong>Container</strong>{text(row["Container Technology"]) || "—"}</p></div></section><BaselineConfigurationRelationships occurrenceId={occurrenceId} productId={row.__meta.productId} /></> : null}
      {tab === "quality" ? <section className="domain-section"><div className="section-heading"><h3>{quality.label} automated checks</h3><span>Calculated indicators; manual review and evidence are not exported in the A2O Tech Stack XLSX format</span></div>{quality.issues.length ? <ul className="quality-checks">{quality.issues.map((issue, index) => <li key={`${issue.field}:${index}`} className={`quality-${issue.severity}`}><strong>{issue.field}</strong><span>{issue.message}</span></li>)}</ul> : <p className="quality-complete">The current Release baseline record passes all configured checks.</p>}</section> : null}
    </DomainPageShell>
  );
}
