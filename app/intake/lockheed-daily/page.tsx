...
export default function LockheedDailyIntakePage() {
  ...
  async function selectFiles(list?: FileList | null) {
    ...
    setBusy(true); setMessage(""); setPreview(null); setDecisions({}); setTargets({});
    try {
  ...
  function changeDataset(fileId: string, dataset: LockheedDailyDataset) {
    ...
    setPreview(null); setDecisions({}); setTargets({});
  }
    ...
    try {
      ...
      if (payload.preview) {
        ...
        if (mode === "preview") {
          ...
          setTargets(Object.fromEntries(payload.preview.items.map((item) => [item.id, item.proposedTargetId || ""])));
        }
      }
      ...
      if (mode === "apply") {
        ...
        await loadState();
      } else setMessage("Preview complete. Verify each file, row decision, and optional canonical trace link.");
