...
export default function IntakePage() {
  ...
  return <DomainPageShell title="Import & Data Quality" subtitle="A2O intake snapshots, automated checks, and analyst review." releaseScope={active ? `Active intake: ${active.fileName}` : "No active intake"} contextMode="portfolio" actions={<Link className="primary-button" href="/">Open baseline records</Link>}>
    ...
    <section className="split-layout"><article className="domain-card"><span className="eyebrow">A2O EXCHANGE CONTRACT</span><h3>Exact 24-column XLSX format</h3><p>The application database is the editable Working Technical Baseline. Each imported A2O workbook is retained as an immutable intake snapshot for reconciliation and rollback; import does not designate it as an official LM or Government record.</p><p className="entity-actions"><Link href="/">Import workbook or edit records</Link><Link href="/pbs">Open product structure</Link></p></article><article className="domain-card"><span className="eyebrow">QUALITY AND REVIEW</span><h3>Two separate indicators</h3><p><strong>Automated checks</strong> identify missing or inconsistent baseline values. <strong>Analyst review</strong> records that a person assessed the current baseline record. Neither is included in the A2O XLSX export.</p><p className="entity-actions"><Link href="/">Open quality and review controls</Link></p></article></section>
