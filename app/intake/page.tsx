...
export default function IntakePage() {
  ...
  return <DomainPageShell title="Import Hub & Quality" subtitle="Choose the source you received. Preview, reconcile, and approve before any governed record is updated." releaseScope={active ? `Current A2O import: ${active.fileName}` : "No A2O workbook imported"} contextMode="portfolio" actions={<><Link className="ghost-button" href="/intake/lockheed-daily">Lockheed daily delivery</Link><Link className="primary-button" href="/intake/a2o">Import A2O workbook</Link></>}>
    <section className="decision-principle"><strong>Import control</strong><span>Each import retains its source receipt and compares incoming values to the canonical record. Valid source identities create or refresh canonical objects automatically. Review is for approval, skips, and genuine duplicate or invalid identities. A missing row never silently deletes a governed record.</span></section>
    ...
    <section className="import-hub-grid">
      ...
      <article className="domain-card import-source-card"><span className="eyebrow">MILWAUKEE COMPONENT DEPLOYMENT SOFTWARE</span><h3>CD SW deployment matrix</h3><p><strong>Use for:</strong> machine identities, software catalog records, and the X-marked application-to-server installation matrix.</p><p><strong>Expected:</strong> the delivered CSV/XLS/XLSX with Machine Type, UUID, Hostname, and ID rows above the software table.</p><p>Choose the owning Platform and Release; blank spacer rows and columns do not require cleanup.</p><Link className="primary-button" href="/intake/cd-sw">Import CD SW matrix</Link></article>
