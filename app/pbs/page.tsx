...
export default function PbsPage() {
  ...
  return <DomainPageShell title="Product Structure" subtitle="Products organized by release and deployment location." releaseScope={release === "all" ? "All releases" : release} actions={<><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find product, alias, or source key" /></label><select value={release} onChange={(event) => setRelease(event.target.value)}><option value="all">All releases</option>{releases.map((item) => <option key={item}>{item}</option>)}</select></>}>
    <section className="kpi-grid" aria-label="Product structure summary"><div className="kpi-card"><span>Products</span><strong>{products.length}</strong><small>Reported in the baseline</small></div><div className="kpi-card"><span>Releases</span><strong>{release === "all" ? releases.length : 1}</strong><small>Selected release scope</small></div><div className="kpi-card"><span>Baseline records</span><strong>{products.reduce((total, product) => total + product.rows.length, 0)}</strong><small>Product-release records</small></div><div className="kpi-card"><span>Configuration paths</span><strong>{uniqueConfiguration}</strong><small>Tier / resource / host combinations</small></div></section>
