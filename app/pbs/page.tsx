...
type ProductNode = { id: string; label: string; rows: ReturnType<typeof useBaselineWorkspace>["rows"] };

export default function PbsPage() {
  const { rows, loading, error } = useBaselineWorkspace();
  const [release, setRelease] = useState("all");
  const [query, setQuery] = useState("");
  const releases = useMemo(() => Array.from(new Set(rows.map((row) => String(row.ReleaseName || "").trim()).filter(Boolean))).sort(), [rows]);
  const products = useMemo(() => {
    const selectedRows = rows.filter((row) => (release === "all" || String(row.ReleaseName || "").trim() === release) && `${row.LongName || ""} ${row.ShortName || ""} ${row["#"] || ""}`.toLowerCase().includes(query.trim().toLowerCase()));
