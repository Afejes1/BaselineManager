...
export default function PbsPage() {
  ...
  return <DomainPageShell title="Product Deployment Structure" subtitle="Reported Products organized by release and configuration placement." releaseScope={releaseLens || "All releases"} contextMode="filter" actions={<label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find product, alias, or source key" /></label>}>
    <section className="decision-principle"><strong>Structure boundary</strong><span>This is a deployment structure derived from A2O records. It does not claim a Product Breakdown Structure of internal components; component relationships require an authoritative source before they are governed here.</span></section>
    <section className="kpi-grid" aria-label="Product structure summary"><div className="kpi-card"><span>Products</span><strong>{products.length}</strong><small>Reported in the baseline</small></div><div className="kpi-card"><span>Releases</span><strong>{releaseLens ? 1 : releases.length}</strong><small>Selected release scope</small></div><div className="kpi-card"><span>Baseline records</span><strong>{products.reduce((total, product) => total + product.rows.length, 0)}</strong><small>Product-release records</small></div><div className="kpi-card"><span>Configuration paths</span><strong>{uniqueConfiguration}</strong><small>Tier / resource / host combinations</small></div></section>
