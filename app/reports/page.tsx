...
type ReportTab = "inventory" | "release" | "funding" | "objectives" | "decomposition";
...
export default function ReportsPage() {
  ...
  return <DomainPageShell title="Leadership Reports" subtitle="What is where, what changed, and which Change Requests need a decision." releaseScope={`${releases.length} releases`} contextMode="comparison" actions={<><button className="ghost-button" type="button" disabled={exportBlocked} title={exportBlockReason || "Print the complete governed report"} onClick={printReport}>Print report</button><button className="primary-button" type="button" disabled={exportBlocked} title={exportBlockReason || "Export the complete governed report workbook"} onClick={exportReport}>Export workbook</button></>}>
    ...
    <nav className="detail-tabs report-tabs" aria-label="Report views"><button className={tab === "inventory" ? "active" : ""} onClick={() => setTab("inventory")}>WHAT · WHERE · WHEN</button><button className={tab === "release" ? "active" : ""} onClick={() => setTab("release")}>Release change book</button><button className={tab === "funding" ? "active" : ""} onClick={() => setTab("funding")}>Funding portfolio</button><button className={tab === "objectives" ? "active" : ""} onClick={() => setTab("objectives")}>Objective delivery</button><button className={tab === "decomposition" ? "active" : ""} onClick={() => setTab("decomposition")}>Change Request decomposition</button></nav>
    ...
    {tab === "objectives" ? <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">OBJECTIVE DELIVERY REGISTER</span><h3>Contractor work, technical scope, and completion evidence</h3></div><Link href="/objectives">Manage LM Objectives</Link></div><div className="domain-table-wrap"><table><thead><tr><th>Objective</th><th>Accountable / reported Change Requests</th><th>Target / status</th><th>Technical scope</th><th>Requirements / acceptance</th><th>Estimate variance</th></tr></thead><tbody>{objectiveRows.map((item) => <tr key={item.ObjectiveRecordId}><td><Link href={`/objectives/${encodeURIComponent(item.ObjectiveRecordId)}`}><strong>{item.ObjectiveID}</strong><br />{item.Objective}</Link><small>{item.TechnicalOwner} · source {item.SourceAsOf}</small></td><td><strong>{item.AccountableRequest}</strong><small>Reported: {item.ReportedRequests}</small><small>{item.FundingDecision}</small></td><td>{item.TargetRelease}<small>{readable(item.LMStatus)} · finish {item.PlannedFinish}</small></td><td>{item.TechnicalEffects} attributed effects<small>{item.DownstreamRequests} downstream requests</small></td><td>{item.RequirementsDispositioned}/{item.Requirements} requirements<small>{item.AcceptancePassed}/{item.AcceptanceCriteria} criteria accepted</small></td><td>LM {item.IncumbentHoursLikely ?? "—"}<small>Government {item.GovernmentHoursLikely ?? "—"}</small></td></tr>)}{!objectiveRows.length ? <tr><td colSpan={6} className="empty">No LM Objectives are available.</td></tr> : null}</tbody></table></div></section> : null}
    {tab === "decomposition" ? <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">CHANGE REQUEST DECOMPOSITION</span><h3>Funding decision → LM Objectives → effects → acceptance</h3></div><span>{changes.requests.length} Change Requests</span></div><div className="domain-list">{changes.requests.map((request) => { const related = decisions?.objectives.filter((item) => objectiveIsRelatedToChangeRequest(item, request.id, decisions?.objectiveChangeRequestLinks || [])) || []; return <article className="domain-card" key={request.id}><div className="section-toolbar"><div><span className={`decision-badge decision-${request.decisionStatus}`}>{request.decisionStatus}</span><h3><Link href={`/changes/${encodeURIComponent(request.id)}`}>{request.externalIdentifier} · {request.title}</Link></h3></div><span>{request.requestedReleaseName || "No release"}</span></div>{related.length ? related.map((objective) => { const effects = decisions?.objectiveEffectAttributions?.filter((item) => item.objectiveId === objective.id).length || 0; const criteria = decisions?.criteria.filter((item) => item.objectiveId === objective.id) || []; const relationship = objective.changeRequestId === request.id ? "Accountable" : "Reported"; return <div className="wbs-branch" key={objective.id}><strong><Link href={`/objectives/${encodeURIComponent(objective.id)}`}>{objective.externalIdentifier} · {objective.title}</Link></strong><span>{relationship} reference · {readable(objective.status)} · {effects} effects · {criteria.filter(criterionIsAccepted).length}/{criteria.length} acceptance complete</span></div>; }) : <p className="empty">No LM Objectives are linked to this Change Request.</p>}</article>; })}</div></section> : null}
  </DomainPageShell>;
}
