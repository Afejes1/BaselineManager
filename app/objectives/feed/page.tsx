...
export default function ObjectiveFeedPage() {
  ...
  const resolutions = useMemo(() => importResolutions(reviewItems, reviewDecisions, {}), [reviewDecisions, reviewItems]);
  ...
  const graphBrowser = useCollectionBrowser(graph, (item) => `${item.row.externalRecordKey} ${item.row.externalIdentifier} ${item.row.title} ${item.row.jpoIdentifiers.join(" ")} ${item.row.domains.join(" ")} ${item.row.release} ${item.links.map((link) => `${link.type} ${link.target}`).join(" ")}`);
  const subjectBrowser = useCollectionBrowser(subjects, (subject) => `${subject.feed_key} ${subject.jira_identifier || ""} ${subject.title || ""} ${(subject.jpoLinks || []).map((link) => link.external_identifier).join(" ")} ${subject.canonical_objective_title || ""} ${subject.presentInLatestSnapshot ? "present" : "absent"}`);
  ...
  async function selectFile(file: File | undefined) {
    if (!file) return;
    setNotice(""); setPreview(null); setReviewDecisions({}); setFileName(file.name);
  ...
  async function reconcile(mode: "preview" | "apply") {
    ...
    try {
      ...
      else setNotice("Preview complete. Review source changes before applying this snapshot. Normal source identities are materialized automatically.");
  ...
  async function reconcileSubject(subjectId: string, canonicalObjectiveId: string) {
    ...
    try {
      ...
      setNotice(`Source identity corrected to ${payload.canonicalObjectiveTitle || "the canonical LM Objective"}. Accountability and JPO/MCP references were not changed.`);
      await loadFeedState();
  ...
  return <DomainPageShell title="Lockheed Objective Feed" subtitle="Import a daily GitLab Pages JSON snapshot. The feed is retained external evidence, not Government delivery authority." releaseScope={`${history.length} retained snapshots`} actions={<><Link className="ghost-button" href="/intake">Import Hub</Link><Link className="ghost-button" href="/objectives">LM Objectives</Link><Link className="ghost-button" href="/objectives/import">Structured workbook</Link></>}>
    ...
    /> : null}
    {graph.length ? <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">OBJECTIVE DEPENDENCY MAP</span><h3>Lockheed-reported block relationships</h3></div><span>{preview || records.length ? "Previewed file" : "Latest applied source snapshot"} · Click an Objective to inspect its reported values.</span></div><div className="feed-graph-layout"><div className="feed-graph-panel"><CollectionControls browser={graphBrowser} itemLabel="Objectives" placeholder="Find Jira ID, title, JPO/MCP, domain, or dependency" /><div className="feed-graph" role="list" aria-label="Lockheed Objective dependency map">{graphBrowser.pageItems.map(({ row, links }) => <button type="button" key={row.externalRecordKey} className={`feed-node ${selected?.externalRecordKey === row.externalRecordKey ? "feed-node-selected" : ""} status-${row.disposition}`} onClick={() => setSelectedKey(row.externalRecordKey)}><span className="record-type">{row.externalIdentifier || `RECORD ${row.externalRecordKey}`}</span><strong>{row.title || "Untitled Objective"}</strong><small>{row.jpoIdentifiers.length ? `JPO / MCP: ${row.jpoIdentifiers.join(", ")}` : "No JPO / MCP supplied"}</small><span className="feed-node-links">{links.length ? links.map((link) => <i key={`${link.type}:${link.target}`} className={link.known ? "known" : "unresolved"}>{link.type} → {link.target}</i>) : "No reported block relationships"}</span></button>)}{!graphBrowser.pageItems.length ? <p className="empty">No Objectives match the current map view.</p> : null}</div><CollectionPager browser={graphBrowser} itemLabel="Objectives" /></div><aside className="feed-inspector">{selected ? <><span className="eyebrow">SELECTED SOURCE RECORD</span><h3>{selected.externalIdentifier || `Record ${selected.externalRecordKey}`}</h3><p>{selected.title}</p><dl className="record-facts"><div><dt>JPO / MCP</dt><dd>{selected.jpoIdentifiers.join(", ") || "Not supplied"}</dd></div><div><dt>Domains</dt><dd>{selected.domains.join(", ") || "Not supplied"}</dd></div><div><dt>Schedule</dt><dd>{selected.targetStart || "—"} → {selected.targetFinish || "—"}</dd></div><div><dt>ROM / completion</dt><dd>{selected.rom || "—"} / {selected.percentComplete ?? "—"}%</dd></div><div><dt>Release / funding</dt><dd>{selected.release || "—"} / {selected.funding || "—"}</dd></div></dl><h4>Snapshot change</h4>{selected.diffs.length ? <ul className="source-diff-list">{selected.diffs.map((diff) => <li key={diff.field}><strong>{diff.field}</strong><del>{diff.before || "(blank)"}</del><ins>{diff.after || "(blank)"}</ins></li>)}</ul> : <p className="entity-meta">No prior source difference for this record.</p>}{selected.issues.length ? <p className="warning-copy">{selected.issues.join(" ")}</p> : null}{selected.url ? <p className="entity-meta">External source reference retained as text; outbound navigation is disabled in this runtime: <span className="source-reference">{selected.url}</span></p> : null}</> : <p>Select an Objective to inspect it.</p>}</aside></div></section> : null}
    <section className="domain-section">
      <div className="section-toolbar"><div><span className="eyebrow">CANONICAL OBJECTS CREATED FROM SOURCE</span><h3>Imported Objectives and reported Change Request references</h3></div><button className="ghost-button" type="button" onClick={() => void loadFeedState()}>Refresh feed state</button></div>
      <p className="entity-meta">A valid Jira ID creates or refreshes its canonical LM Objective automatically. JPO/MCP values create or link reported Change Request references. Neither action assigns accountability, funding, or delivery authority.</p>
      <CollectionControls browser={subjectBrowser} itemLabel="source subjects" placeholder="Filter Jira ID, feed key, JPO/MCP, title, or canonical link" />
      <div className="review-table-viewport source-subject-viewport feed-subject-table"><table><thead><tr><th>Lockheed source subject</th><th>Reported JPO / MCP</th><th>Latest source state</th><th>Canonical LM Objective</th></tr></thead><tbody>{subjectBrowser.pageItems.map((subject) => <tr key={subject.id}><td><strong>{subject.jira_identifier || `Record ${subject.feed_key}`}</strong><small>Feed key {subject.feed_key} · {subject.title || "No title supplied"}</small></td><td>{subject.jpoLinks?.length ? subject.jpoLinks.map((link) => link.change_request_id ? <Link key={`${subject.id}:${link.external_identifier}`} href={`/changes/${encodeURIComponent(link.change_request_id)}`}>{link.change_request_external_identifier || link.external_identifier}</Link> : <span className="source-reference" key={`${subject.id}:${link.external_identifier}`}>{link.external_identifier}</span>) : <span className="entity-meta">Not supplied</span>}</td><td><span className={`status-pill ${subject.presentInLatestSnapshot ? "status-accepted" : "status-warning"}`}>{subject.presentInLatestSnapshot ? "PRESENT" : "ABSENT"}</span><small>{subject.presentInLatestSnapshot ? "In latest applied file" : "Not in latest applied file; history retained"}</small></td><td>{subject.canonical_objective_id ? <div className="trace-link-cell"><Link href={`/objectives/${encodeURIComponent(subject.canonical_objective_id)}`}>{subject.canonical_objective_title || "Open canonical Objective"}</Link><details><summary>Correct identity link</summary><p className="entity-meta">Use only if Lockheed reused or corrected an external identity. This does not change accountability or reported JPO/MCP references.</p><select aria-label={`Correct canonical Objective identity for ${subject.jira_identifier || subject.feed_key}`} value={linkChoice[subject.id] || ""} onChange={(event) => setLinkChoice({ ...linkChoice, [subject.id]: event.target.value })}><option value="">Choose an existing canonical Objective</option>{objectiveOptions.map((objective) => <option key={objective.id} value={objective.id}>{objective.externalIdentifier} · {objective.title}</option>)}</select>{linkChoice[subject.id] ? <button className="ghost-button compact-button" type="button" disabled={linkingSubjectId === subject.id} onClick={() => void reconcileSubject(subject.id, linkChoice[subject.id])}>{linkingSubjectId === subject.id ? "Correcting…" : "Correct link"}</button> : null}</details></div> : <div className="trace-link-cell"><strong>Identity exception</strong><small>This older or incomplete source subject has no canonical Objective link. Select the existing Objective only after confirming the source identity.</small><select aria-label={`Resolve canonical Objective identity for ${subject.jira_identifier || subject.feed_key}`} value={linkChoice[subject.id] || ""} onChange={(event) => setLinkChoice({ ...linkChoice, [subject.id]: event.target.value })}><option value="">Choose canonical Objective</option>{objectiveOptions.map((objective) => <option key={objective.id} value={objective.id}>{objective.externalIdentifier} · {objective.title}</option>)}</select><button className="ghost-button compact-button" type="button" disabled={!linkChoice[subject.id] || linkingSubjectId === subject.id} onClick={() => void reconcileSubject(subject.id, linkChoice[subject.id])}>{linkingSubjectId === subject.id ? "Correcting…" : "Resolve identity"}</button></div>}</td></tr>)}{!subjectBrowser.pageItems.length ? <tr><td colSpan={4} className="empty">No Lockheed source subject matches the current view.</td></tr> : null}</tbody></table></div>
      <CollectionPager browser={subjectBrowser} itemLabel="source subjects" />
    </section>
