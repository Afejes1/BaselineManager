...
export default function ProductDetailPage() {
  ...
  return <DomainPageShell title={`Product: ${canonical}`} subtitle="Canonical identity, Release fielding history, deployments, decisions, delivery, and evidence." releaseScope={productRows.length ? `${productRows.length} Release records · ${releases.length} releases` : "Not fielded in a Release"} contextMode="record" objectContext={objectContext} actions={<><label className="search" style={{ width: "260px" }}><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search placements" /></label><AnalyticsLink kind="product" id={canonicalProductId || productId} />{masterProduct ? <Link className="ghost-button" href="/stewardship">Merge duplicate</Link> : null}{masterProduct ? <button className="ghost-button" type="button" onClick={() => setEditing(true)}>Edit Product</button> : null}</>}>
    <section className="summary"><div className="metric"><span>Release records</span><strong>{productRows.length}</strong><small>{productRows.length ? `Across ${releases.length} releases` : "Create a Release record to establish fielding"}</small></div><div className="metric"><span>Platforms</span><strong>{productPlatforms.length}</strong><small>{productRows.length ? `${tiers.length} tiers · ${resources.length} resources · ${hosts.length} hosts` : "Not assessed until fielded"}</small></div><div className="metric"><span>Supplier</span><strong>{supplier || "Unassigned"}</strong><small>{classifications.join(" · ") || "Classification not recorded"}</small></div><div className="metric metric-alert"><span>Change Requests</span><strong>{changeRequests.length}</strong><small>{changeRequests.filter((item) => item.decisionStatus === "pending").length} funding decisions pending</small></div></section>
    ...
    {tab === "installations" ? <section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">GOVERNED INSTALLATIONS</span><h3>Where this Product is installed by Release</h3></div><Link className="mini-action" href="/topology">Open deployment topology →</Link></div><p className="entity-meta">This is the normalized system configuration. Operating systems, hypervisors, applications, and supporting software use the same canonical Product catalog.</p><div className="domain-table-wrap"><table><thead><tr><th>Release</th><th>Platform</th><th>Infrastructure node</th><th>Role</th><th>Version / instance</th><th>Baseline record</th></tr></thead><tbody>{infrastructureInstallations.map((installation) => { const state = infrastructureStateById.get(installation.nodeStateId); const node = state ? infrastructureNodeById.get(state.infrastructureNodeId) : undefined; const platform = infrastructurePlatformById.get(installation.platformId); const release = infrastructureReleaseById.get(installation.releaseId); return <tr key={installation.id}><td>{release ? <Link href={`/releases/${encodeURIComponent(release.name)}`}>{release.name}</Link> : "Unknown Release"}</td><td>{platform ? <Link href={`/platforms/${encodeURIComponent(platform.id)}`}>{platform.code} · {platform.name}</Link> : "Unknown Platform"}</td><td><strong>{node?.code || "Unknown node"}</strong><small>{node?.name || "Identity not found"}</small></td><td>{installation.installationRole.replaceAll("_", " ")}</td><td>{installation.version || "Not recorded"}<small>{installation.instanceName || installation.deploymentStatus}</small></td><td>{installation.baselineOccurrenceId ? <Link href={`/occurrences/${encodeURIComponent(installation.baselineOccurrenceId)}`}>{installation.sourceKey || "Open record"}</Link> : "No baseline-row link"}</td></tr>; })}{!infrastructureInstallations.length ? <tr><td colSpan={6} className="empty">No governed infrastructure placement is recorded for this Product. Add it from the applicable Platform Release configuration.</td></tr> : null}</tbody></table></div></section> : null}
