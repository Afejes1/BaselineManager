...
export default function ProductDetailPage() {
  ...
  return <DomainPageShell title={`Product: ${canonical}`} subtitle="Canonical identity, Release fielding history, deployments, decisions, delivery, and evidence." releaseScope={productRows.length ? `${productRows.length} Release records · ${releases.length} releases` : "Not fielded in a Release"} contextMode="record" objectContext={objectContext} actions={<><label className="search" style={{ width: "260px" }}><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search placements" /></label>{masterProduct ? <Link className="ghost-button" href="/stewardship">Merge duplicate</Link> : null}{masterProduct ? <button className="ghost-button" type="button" onClick={() => setEditing(true)}>Edit Product</button> : null}</>}>
