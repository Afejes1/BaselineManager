...
export default function TopologyPage() {
  ...
  return <DomainPageShell title="Deployment Topology" subtitle="Where products are installed for the selected release." releaseScope={effectiveReleaseName || "Select a release"} contextMode="filter">
    {loading ? <section className="domain-section"><p className="empty">Loading deployment records…</p></section> : error ? <section className="domain-section"><p className="error-copy">{error}</p></section> : !effectiveReleaseName ? <section className="domain-section empty-state"><h3>Select a release lens</h3><p>Deployment topology is a release-specific view. Set the Release Lens in the header.</p><Link href="/releases">Open releases</Link></section> : <><section className="kpi-grid" aria-label="Topology overview"><div className="kpi-card"><span>Baseline hosts</span><strong>{overview.configurationNodes}</strong><small>Tier / resource / host placements</small></div><div className="kpi-card"><span>Products</span><strong>{overview.products}</strong><small>Present in this release</small></div><div className="kpi-card"><span>Location details</span><strong>{extensions.hostProfiles.length}</strong><small>Additional modeled properties</small></div><div className="kpi-card"><span>Deployment details</span><strong>{extensions.deploymentProfiles.length}</strong><small>Additional modeled properties</small></div></section><section className="domain-section"><div className="section-toolbar"><div><span className="eyebrow">HOW TO READ THIS VIEW</span><h3>Shared foundation and conditional deployment layers</h3></div><Link href={`/releases/${encodeURIComponent(effectiveReleaseName)}`}>Open release</Link></div><p className="entity-meta">One foundation is shown for the same reported location context. VM, runtime, and container-instance layers appear only when the record supports them. Blank source values remain a data gap; they are not displayed as invented infrastructure.</p>{extensionLoading ? <p className="empty">Loading modeled details…</p> : extensionError ? <p className="error-copy">{extensionError}</p> : <TopologyStack releaseName={effectiveReleaseName} rows={scopedRows} hostProfiles={extensions.hostProfiles} deploymentProfiles={extensions.deploymentProfiles} />}</section></>}
  </DomainPageShell>;
}
